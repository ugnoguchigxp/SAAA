...
fn status_of(state: &AppState, id: &str) -> (Status, Option<FireResult>) {
    state
        .sqlite_writer
...
fn messages(state: &AppState) -> Vec<String> {
    state
        .sqlite_writer
        .read_serialized(|connection| {
...
#[test]
fn sl_05_task_run_without_dispatch_receipt_is_not_started() {
    let state = state();
...
#[test]
fn sl_07_b_asks_without_delegation() {
    let state = state();
    tick::enable(&state, 100).unwrap();
...
#[test]
fn sl_08_c_holds_on_situation_then_digest() {
    let state = state();
    tick::enable(&state, 100).unwrap();
    state
        .situation
...
#[test]
fn sl_05_d_missed_after_window() {
    let state = state();
...
#[test]
fn sl_06_e_crash_closes_firing_without_rerun() {
    let state = state();
    tick::enable(&state, 100).unwrap();
...
#[test]
fn sl_06_reminder_firing_is_closed_without_inspect() {
    let state = state();
    tick::enable(&state, 100).unwrap();
...
#[test]
fn sl_05_f_time_change_supersedes_old() {
    let state = state();
    tick::enable(&state, 100).unwrap();
...
#[test]
fn sl_05_g_withdrawn_does_not_fire() {
    let state = state();
    tick::enable(&state, 100).unwrap();
...
#[test]
fn sl_09_defers_when_generation_slot_busy() {
    let state = state();
    tick::enable(&state, 100).unwrap();
...
#[test]
fn sl_21_r_disable_stops_tick_and_closes_firing() {
    let state = state();
    tick::enable(&state, 100).unwrap();
    insert_due(&state, "r1", 50, Some("del-1"), None);
    state
...
#[test]
fn sl_04_due_p95_under_five_ms() {
    let connection = Connection::open_in_memory().unwrap();
...
#[test]
fn sl_23_tick_p95_under_fifteen_ms() {
    let state = state();
...
#[test]
fn sl_19_o_forget_clears_payload_and_summary() {
    let state = state();
    state
        .sqlite_writer
        .write(|connection| {
...
#[test]
fn sl_20_s_restricted_title_is_reference_only() {
    let entry = Entry {
        id: "s1".into(),
        kind: Kind::Reminder,
...
async fn sl_23_h_i_n_p_calendar_fake_http() {
...
#[test]
fn sl_18_l_m_conflict_and_foreign_notify_once() {
    let state = state();
    state
        .sqlite_writer
        .write(|connection| {
...
#[test]
fn sl_17_k_delegated_delete_asks() {
    let state = state();
    state
        .sqlite_writer
        .write(|connection| {
...
#[test]
fn sl_10_ipc_origin_is_user_explicit() {
    let state = state();
    ...
    let listed = state
        .sqlite_readers
        .read(|connection| super::ledger::list_all(connection, 10))
        .unwrap();
