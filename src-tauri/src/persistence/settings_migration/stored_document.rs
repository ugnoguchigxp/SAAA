...
pub(crate) fn initialize_revision(connection: &Connection) -> rusqlite::Result<()> {
...
pub(crate) fn migrate_provider_document(value: &mut Value, system_voice: &str, legacy_shape: bool) {
...
pub(super) fn remove_direct_qwen_provider(providers: &mut Value) {
...
pub(super) fn clear_direct_qwen_route(routing: &mut Value) {
    let Some(conversation) = routing
        .get_mut("conversationRespond")
        .and_then(Value::as_object_mut)
    else {
...
pub(super) fn retarget_direct_qwen_actors(roles: &mut Value) {
...
pub(super) fn migrate_agent_document(value: &mut Value) {
    let Some(document) = value.as_object_mut() else {
        return;
    };
    document
