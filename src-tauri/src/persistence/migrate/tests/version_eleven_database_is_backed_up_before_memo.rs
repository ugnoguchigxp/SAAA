use super::*;
#[test]
    pub(super) fn version_eleven_database_is_backed_up_before_memory_v12() {
...
#[test]
    pub(super) fn settings_v9_is_backed_up_before_v10_shape_migration() {
        let directory = tempfile::tempdir().expect("temporary directory");
...
#[test]
    pub(super) fn current_database_with_a_missing_settings_table_is_backed_up_before_repair() {
        let directory = tempfile::tempdir().expect("temporary directory");
...
#[test]
    pub(super) fn version_six_runtime_rows_gain_nullable_supervisor_columns() {
...
#[test]
    pub(super) fn startup_drops_meeting_product_tables() {
        let connection = Connection::open_in_memory().expect("database opens");
        initialize_database(&connection).expect("schema initializes");
...
#[test]
    pub(super) fn version_three_database_migrates_to_four_without_losing_mvp_zero_state() {
        let directory = tempfile::tempdir().expect("temporary directory");
