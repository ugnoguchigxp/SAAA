...
use serde_json::json;
#[test]
pub(super) fn normalizes_regressed_dynamic_lan_provider_id_and_routes() {
...
#[test]
pub(super) fn migration_creates_default_documents() {
    let connection = Connection::open_in_memory().expect("in-memory sqlite");
    initialize_database(&connection).expect("migration succeeds");
    let documents = list_settings_documents(&connection).expect("documents load");
...
#[test]
pub(super) fn existing_provider_settings_gain_the_medium_reasoning_default() {
...
#[test]
pub(super) fn existing_provider_settings_remove_the_legacy_output_token_setting() {
    let connection = Connection::open_in_memory().expect("database opens");
    initialize_database(&connection).expect("initial schema");
    connection
        .execute(
...
#[test]
pub(super) fn existing_voice_settings_copy_the_dynamic_lan_host_once() {
    let connection = Connection::open_in_memory().expect("database opens");
    initialize_database(&connection).expect("initial schema");
    connection
        .execute_batch(
...
#[test]
pub(super) fn existing_voice_settings_gain_japanese_language_registration() {
    let connection = Connection::open_in_memory().expect("database opens");
    initialize_database(&connection).expect("initial schema");
    connection
            .execute(
    ...
    let (schema_version, listening_enabled, allowed_languages): (i64, bool, String) = connection
        .query_row(
...
#[test]
pub(super) fn existing_agent_settings_gain_the_default_agent_name() {
    let connection = Connection::open_in_memory().expect("database opens");
    initialize_database(&connection).expect("initial schema");
    connection
        .execute(
...
#[test]
pub(super) fn existing_agent_settings_gain_an_empty_user_name() {
    let connection = Connection::open_in_memory().expect("database opens");
    initialize_database(&connection).expect("initial schema");
    connection
        .execute(
...
#[test]
pub(super) fn pristine_previous_provider_defaults_migrate_to_dynamic_lan() {
    let connection = Connection::open_in_memory().expect("database opens");
    initialize_database(&connection).expect("initial schema");
...
#[test]
pub(super) fn direct_dynamic_lan_endpoint_migrates_to_host_only_discovery() {
    let connection = Connection::open_in_memory().expect("database opens");
    initialize_database(&connection).expect("initial schema");
    let direct = json!({
        "providers": [{
