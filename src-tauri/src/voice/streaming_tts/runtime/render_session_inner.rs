use super::*;
pub(crate) async fn render_session_inner(
...
pub(crate) async fn render_http_session(
    receiver: &mut mpsc::Receiver<SpeechWork>,
...
pub(crate) async fn resolve_render_route(
    route: &TtsRoute,
...
pub(crate) fn finalize_rendered_chunk(
    sequence: u64,
    boundary_at: Instant,
    path: PathBuf,
    started: Instant,
...
pub(crate) fn adaptive_render_concurrency(samples: &VecDeque<(u64, u64)>) -> usize {
    if samples.is_empty() {
        return 2;
    }
...
pub(crate) fn render_slots_used(rendering: usize, ready: usize, playing: bool) -> usize {
    rendering
        .saturating_add(ready)
        .saturating_add(usize::from(playing))
}
pub(crate) fn wave_duration_ms(path: &Path) -> Result<u64, String> {
    let mut file =
