...
pub(super) fn enrollment_requires_five_samples_of_at_least_ten_seconds() {
    ...
    assert!(
        validate_enrollment_input(&input, &vec![0.1; CANONICAL_SAMPLE_RATE as usize * 10],).is_ok()
    );
    assert!(
        validate_enrollment_input(&input, &vec![0.1; CANONICAL_SAMPLE_RATE as usize * 10 - 1],)
            .is_err()
    );
...
pub(super) fn startup_reconciliation_removes_only_obsolete_owned_voice_files() {
    ...
    let old_temporary = profile.join("voice_sample_old.wav.tmp-0123456789abcdef0123456789abcdef");
