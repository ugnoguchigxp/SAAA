use super::*;
pub(super) fn review_flow_fixture() -> Connection {
...
pub(super) fn final_flow_fixture() -> Connection {
    let c = Connection::open_in_memory().expect("db");
    c.execute_batch("PRAGMA foreign_keys=ON; CREATE TABLE conversations(id TEXT PRIMARY KEY); CREATE TABLE runtime_runs(id TEXT PRIMARY KEY); CREATE TABLE conversation_messages(id TEXT PRIMARY KEY,conversation_id TEXT,role TEXT,content TEXT,created_at TEXT); INSERT INTO conversations VALUES('c'); INSERT INTO runtime_runs VALUES('run');").expect("base");
    crate::role_routing::schema::migrate(&c).expect("schema");
    crate::role_routing::learning::schema::migrate(&c).expect("learning");
...
#[test]
pub(super) fn rr_29_cancel_completion_both_orders() {
...
pub(super) fn review_issue(verdict: &str) -> crate::role_routing::review::ReviewIssue {
...
#[test]
pub(super) fn rr_25_decision_consumed_once_and_claims_revise_atomically() {
...
#[test]
pub(super) fn rr_25_no_verified_issue_keeps_the_reviewed_draft() {
...
#[test]
pub(super) fn rr_26_unresolved_review_proposes_and_consumes_premium_once() {
    let c = review_flow_fixture();
...
#[test]
pub(super) fn rr_05_normal_turn_advances_two_steps_without_persisting_the_draft_body() {
    let c = Connection::open_in_memory().expect("db");
...
#[test]
pub(super) fn rr_39_host_receipt_p95_is_under_fifty_milliseconds() {
    let c = Connection::open_in_memory().expect("db");
...
#[test]
pub(super) fn rr_09_activity_has_no_payload_and_stops_at_terminal_root() {
    let c = Connection::open_in_memory().expect("db");
...
#[test]
pub(super) fn rr_12_finish_requires_a_confirmed_step_transition() {
    let c = Connection::open_in_memory().expect("db");
    c.execute_batch("PRAGMA foreign_keys=ON;CREATE TABLE conversations(id TEXT PRIMARY KEY);CREATE TABLE runtime_runs(id TEXT PRIMARY KEY);CREATE TABLE conversation_messages(id TEXT PRIMARY KEY,conversation_id TEXT,role TEXT,content TEXT,created_at TEXT);INSERT INTO conversations VALUES('c');")
        .expect("base");
    crate::role_routing::schema::migrate(&c).expect("schema");
...
#[test]
pub(super) fn rr_04_receipt_rows_rollback_with_the_runtime_transaction() {
