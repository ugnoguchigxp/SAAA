#[cfg(test)]
static SEARCH_CALL_LOG: std::sync::Mutex<Vec<String>> = std::sync::Mutex::new(Vec::new());
...
impl ContextStillSearchClient {
    pub fn from_environment() -> Self {
        Self::with_run_dir(resolve_run_dir(), super::control_plane::memory_enabled())
    }
...
async fn initialize(manifest: EndpointManifest) -> Result<Session, SearchError> {
    let transport =
...
fn map_transport(error: TransportError) -> SearchError {
    match error {
...
fn validate_catalog(result: &Value) -> Result<(), SearchError> {
    let tools = result
        .get("tools")
...
fn input_schema(knowledge: bool) -> Value {
...
fn parse_arguments(tool_name: &str, raw: &str) -> Result<Map<String, Value>, SearchError> {
...
fn valid_array(value: &Value, key: &str) -> bool {
...
fn compact_result(tool_name: &str, result: &Value) -> Result<String, SearchError> {
    let content = result
        .get("content")
        .and_then(Value::as_array)
...
fn compact_item(tool_name: &str, item: &Value) -> Result<Value, SearchError> {
...
fn load_manifest(run_dir: &Path) -> Result<EndpointManifest, SearchError> {
...
fn validate_manifest(manifest: &EndpointManifest) -> Result<(), SearchError> {
...
fn validate_owned_path(path: &Path, directory: bool) -> Result<(), SearchError> {
