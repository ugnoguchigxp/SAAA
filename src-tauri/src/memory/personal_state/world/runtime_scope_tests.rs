...
#[test]
fn m2_05_authorized_meeting_request_succeeds() {
...
fn m2_06_unregistered_target_is_denied_without_leaking() {
    let fixture = Fixture::new(&[("resource", MEETING_ID)]);
    fixture.add_meeting("other", "active", "100", None, None);
    fixture
        .meeting
        .set(Some("other"), crate::meeting::MeetingState::Active);
    let access = fixture.access();
    let request = fixture.request(access, vec![fixture.meeting_ref("other")], None);
    assert_eq!(prepare(&fixture, request), Err(FrameError::ScopeDenied));
}
...
fn m2_06_revoked_scope_is_denied() {
    ...
    fixture
        ...
        .write(|c| {
            c.execute(
                "UPDATE context_scopes SET state='revoked' WHERE scope_key='resource:m1'",
                [],
            )
            .map_err(crate::database_error)?;
            c.execute(
...
fn m2_21_one_disallowed_reference_denies_the_whole_request() {
    ...
    fixture
        ...
        .set(Some(MEETING_ID), crate::meeting::MeetingState::Active);
    let refs = vec![
        fixture.meeting_ref(MEETING_ID),
...
fn m2_21_same_name_in_another_project_is_denied() {
    ...
    fixture
        ...
        .unwrap();
    let access = fixture.access();
    let request = fixture.request(access, vec![fixture.meeting_ref("shadow")], None);
    assert_eq!(prepare(&fixture, request), Err(FrameError::ScopeDenied));
}
