...
pub(super) fn derived_metadata_unit(slice: &WorldSliceV2, source_complete: bool) -> SliceUnitV2 {
...
pub(super) fn push_notice(slice: &mut WorldSliceV2, notice: &str) {
    if !slice.notices.iter().any(|existing| existing == notice) {
        slice.notices.push(notice.to_string());
    }
}
pub(super) fn push_reason(slice: &mut WorldSliceV2, reason: &str) {
