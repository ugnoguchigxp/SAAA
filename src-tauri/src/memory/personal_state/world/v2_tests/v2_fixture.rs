...
pub(crate) fn fixture_v2() -> V2Fixture {
...
#[test]
pub(crate) fn d21_unknown_seed_is_a_notice_not_a_fabricated_node() {
    let f = fixture_v2();
    let slice = run_v2(
        &f.writer,
...
#[test]
pub(crate) fn v2_forgotten_source_version_is_not_revived_by_a_new_version_with_the_same_id() {
    let f = fixture_v2();
    f.writer
        .write(|c| {
            c.execute(
...
#[test]
pub(crate) fn d21_stale_projection_is_an_omission_not_an_error() {
    let writer = writer_db();
    writer
        .write(|c| {
...
#[test]
pub(crate) fn d21_query_is_read_only() {
    let f = fixture_v2();
    let before: u64 = f
        .writer
        .read_serialized(|c| {
...
#[test]
pub(crate) fn d31_two_hop_relation_is_fetched_from_the_second_frontier() {
    let f = fixture_v2();
    let (slice, stats) = f
        .writer
        .read_serialized(|c| {
...
#[test]
pub(crate) fn d32_flags_remove_disabled_relation_kinds() {
    let f = fixture_v2();
...
#[test]
pub(crate) fn d32_goals_flag_removes_goal_relations_and_nodes() {
    let f = fixture_v2();
    let flags = IncludeFlags {
        goals: false,
        ..IncludeFlags::default()
...
#[test]
pub(crate) fn d38_goal_is_reached_through_a_relevance_path() {
    let f = fixture_v2();
    let slice = run_v2(
        &f.writer,
        &[WorldSeed::EntityId("c1".into())],
...
#[test]
pub(crate) fn d38_goal_retraction_removes_current_importance() {
    let f = fixture_v2();
    let mut committer = Committer {
        writer: &f.writer,
        project: PROJECT,
...
#[test]
pub(crate) fn d39_condition_observation_makes_the_relation_satisfied() {
