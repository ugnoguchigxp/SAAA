...
pub(super) fn insert(c: &Connection, id: &str, text: &str) {
    c.execute(
...
#[test]
pub(super) fn inputs_and_jobs_are_atomic_and_all_messages_are_paged() {
    let c = db();
    for n in 0..405 {
...
#[test]
pub(super) fn utf8_loader_preserves_middle_and_tail_without_truncation() {
    let c = db();
    let text = format!(
        "{}中間の訂正{}採用しない",
        "あ".repeat(3000),
...
#[test]
pub(super) fn edit_invalidates_captured_range_and_delete_blocks_resurrection() {
    let c = db();
    insert(&c, "s1", "Aに送る");
...
#[test]
pub(super) fn unconfigured_contract_is_explicit_and_memory_off_preserves_tracking() {
    let c = db();
    insert(&c, "s1", "訂正");
...
#[test]
pub(super) fn one_claim_and_retry_budget_and_epoch_fence() {
    let c = db();
    insert(&c, "s1", "制約");
    let mut t = now();
    for attempt in 0..4 {
...
#[test]
pub(super) fn restore_requires_current_tombstones_and_merges_before_read() {
    let c = db();
    insert(&c, "s1", "忘れる本文");
...
pub(super) fn manifest(c: &Connection) -> generation::Manifest {
...
#[test]
pub(super) fn generation_forget_fences_before_late_results_and_keeps_materialization_separate() {
    let c = db();
    insert(&c, "s1", "入力");
...
#[test]
pub(super) fn derived_answers_and_their_descendants_are_erased_with_the_original() {
    let c = db();
...
#[test]
pub(super) fn expired_views_and_unconfirmed_cancellation_prevent_new_claims() {
    let c = db();
    insert(&c, "s1", "source");
    let mut m = manifest(&c);
    m.expires_at = now() - 1;
...
#[test]
pub(super) fn composer_requires_complete_pending_inputs_and_never_silently_drops_them() {
    let c = db();
    insert(&c, "s1", "送信は禁止");
...
#[tokio::test]
pub(super) async fn partial_messages_stay_candidate_until_full_message_finalization() {
    let c = db();
    insert(
        &c,
        "s1",
...
#[test]
pub(super) fn real_db_backup_restore_merges_current_journal_and_missing_journal_blocks_open() {
...
struct UnrelatedScopeFixture {
    pub(super) writer: std::sync::Arc<SqliteWriter>,
}
#[async_trait::async_trait]
...
#[tokio::test]
pub(super) async fn unrelated_scope_input_does_not_abort_a_scoped_extraction_commit() {
    let connection = db();
