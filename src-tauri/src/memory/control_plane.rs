...
fn load_source_message(connection: &Connection, id: &str) -> Result<SourceMessage, String> {
    connection
        .query_row(
...
fn digest_source_messages(start: &SourceMessage, end: &SourceMessage) -> String {
...
fn require_available_source(connection: &Connection, source_window_id: &str) -> Result<(), String> {
...
const PROFILE_KINDS: &[&str] = &["preference", "communication", "accessibility"];
const WORKING_KINDS: &[&str] = &["open_loop", "commitment", "constraint", "pending_decision"];
const CAPSULE_KINDS: &[&str] = &[
    "active_referent",
    "constraint",
    "open_loop",
    "commitment",
...
fn validate_optional_timestamp(value: Option<&str>) -> Result<(), String> {
...
fn load_items<P: rusqlite::Params>(
    connection: &Connection,
    sql: &str,
    parameters: P,
...
fn trim_observability_events(connection: &Connection, table: &str) -> Result<(), String> {
    let sql = match table {
