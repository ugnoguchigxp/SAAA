...
#[cfg(test)]
pub(crate) const RULE_ITEM_BYTES: usize = 2 * 1_024;
...
pub(super) fn insert_optional_text_array(
...
pub(super) fn input_schema(memory_type: TypedMemoryType) -> Value {
...
pub(super) fn common_input_properties() -> Map<String, Value> {
    Map::from_iter([
        (
            "query".to_string(),
            json!({
...
struct MemoryEnvelope<T> {
    pub(super) contract_version: ContractVersion,
    pub(super) memory_type: TypedMemoryType,
    pub(super) trust: MemoryTrust,
    pub(super) items: Vec<T>,
    pub(super) no_content: bool,
...
#[serde(rename_all = "camelCase", deny_unknown_fields)]
struct MemoryTrust {
    pub(super) trust_class: TrustClass,
    pub(super) instruction_authority: InstructionAuthority,
}
#[derive(Debug, Serialize, Deserialize)]
enum TrustClass {
...
pub(super) fn validate_required_texts<'a>(
...
pub(super) fn validate_optional_text(value: Option<&str>) -> Result<(), TypedRecallContractError> {
