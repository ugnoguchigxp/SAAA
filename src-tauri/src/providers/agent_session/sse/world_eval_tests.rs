...
fn messages(value: &Value) -> &Vec<Value> {
    if let Some(messages) = value.get("messages").and_then(Value::as_array) { return messages; }
...
async fn world_m4a_agent_routes() {
    ...
    for expired in [false, true] {
        ...
        let composed = compose_parts(true, Some(Arc::new(fixture.service())), access.principal, access.policy_revision,
            Some(graph::graph_request("tech")), RUN_ID, &scope, base, vec![], graph::allowed(&scope)).unwrap();
        if expired { fixture.set_now(fixture.now() + 1000); }
        ...
        fixture.writer.write(|c| c.execute("UPDATE ui_settings SET enabled=1", []).map(|_| ()).map_err(crate::database_error)).unwrap();
        ...
        let server = tokio::spawn(async move {
            ...
            for round in 0..2 {
                ...
                let input: Value = serde_json::from_str(body["input"][0]["text"].as_str().unwrap()).unwrap();
                let msgs = messages(&input);
                assert_eq!(msgs.iter().filter(|m| m["role"] == "user").count(), 1);
                let has_world = msgs.iter().any(|m| m["content"].as_str().is_some_and(|s| s.contains("[WORLD_MODEL")));
                ...
                respond(&mut socket, "application/json", &json!({"id":format!("agt_{round}"),"session_id":"ags_test"}).to_string()).await;
                let (mut socket, _) = listener.accept().await.unwrap();
                let (head, _) = request(&mut socket).await;
                assert!(head.starts_with("GET "));
                let event = format!("data: {}\n\ndata: {}\n\n", json!({"type":"message.delta","session_id":"ags_test","turn_id":format!("agt_{round}"),"data":{"text":output}}), json!({"type":"turn.completed","session_id":"ags_test","turn_id":format!("agt_{round}"),"data":{}}));
        ...
        let outcome = run_agent_session_sse(&Client::new(), &provider, &SessionResponse { id:"ags_test".into(), events_url:None }, Url::parse(&format!("{base_url}/v1/agents/sessions/ags_test/events")).unwrap(), &history, 5000, None, ModelStreamContext {
            reasoning_effort:"low", max_output_tokens:256, input:&input, on_event:&Sink::default(), cancellation:Arc::default(), context_health:"green", context_sources:&composed.envelope.selected, context_omissions:&composed.envelope.omitted,
        ...
        assert!(matches!(outcome, ProviderAttemptOutcome::Completed { .. }), "{outcome:?}");
        let bodies = tokio::time::timeout(Duration::from_secs(5), server).await.unwrap().unwrap();
        let selected: Vec<bool> = fixture.writer.read_serialized(|c| {
            let mut s = c.prepare("SELECT EXISTS(SELECT 1 FROM context_generation_inputs i WHERE i.generation_id=g.id AND i.source_kind='world-model' AND i.selected=1) FROM context_generations g ORDER BY ordinal").map_err(crate::database_error)?;
        ...
        report.push(json!({"case_id":if expired {"W12-agent-expired"}else{"W11-agent-initial-followup"},"pass":true,"request_count":2,"world_sent_per_request":selected,"manifest_match":true}));
    }
    println!("WORLD_EVAL_REPORT={}", json!({"schema_version":1,"suite":"world-m4a-agent","cases":report}));
}
