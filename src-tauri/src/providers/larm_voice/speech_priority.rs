...
pub(crate) fn final_ready(
    ...
    if let Some(previous) = previous_final.filter(|previous| previous != run) {
        speech.cancel(&previous);
    }
    if let Some(ack) = state.lfm_run.take() {
        speech.cancel(&ack);
    }
}
...
pub(crate) fn finished(run: &str) {
    ...
    for state in all.values_mut() {
        if state.final_run.as_deref() == Some(run) {
            state.final_run = None;
        }
        if state.lfm_run.as_deref() == Some(run) {
            state.lfm_run = None;
        }
    }
}
...
pub(crate) fn queue_reply(
    ...
    if let Some(previous) = state.lfm_run.replace(run.into()) {
        speech.cancel(&previous);
    }
...
pub(crate) fn stop_conversation(speech: &StreamingSpeechRuntime, conversation: &str) {
    ...
    if let Some(state) = all.get_mut(conversation) {
        state.epoch += 1;
        if let Some(run) = state.lfm_run.take() {
            speech.cancel(&run);
        }
    }
}
