//! ASR's sole conversation entry point. Qwen runs independently after a durable reasoning request.
...
fn record(
    ...
    let reasoning_request: Option<String> = state
        .sqlite_readers
        .read(|c| {
            c.query_row(
                "SELECT reasoning_request_id FROM lfm_voice_utterances WHERE utterance_id=?1",
                [utterance],
                |r| r.get(0),
            )
    ...
    if let Some(id) = reasoning_request {
        attributes.insert(
            "reasoningRequestId".into(),
            audit::AuditAttributeValue::Tag(id),
        );
    }
    let _ = audit::record_frontend_event(
        state,
        &FrontendAuditEventInput {
            component: "conversation".into(),
            event_name: name.into(),
            phase: "state".into(),
            outcome: failure.map(|_| "failure".into()),
            correlation_id: Some(utterance.into()),
            causation_id: Some(utterance.into()),
            conversation_id: Some(conversation.into()),
            runtime_run_id: None,
            session_id: None,
            subject_id: Some(utterance.into()),
...
pub(crate) async fn receive_lfm_utterance(
    ...
    if text.trim().is_empty() || text.chars().count() > 16_000 {
        return Err("lfm-utterance-length-invalid".into());
    }
    ...
    let result = async {
        ...
        let (reasoning_request_id, request_content) = reasoning_request
            .map(|(id, text)| (Some(id), Some(text)))
            .unwrap_or_default();
        Ok::<_, String>(LfmUtteranceResult {
            reasoning_request_id,
            request_content,
            speech_epoch,
            ignored_as_self_speech: false,
    ...
    if let Err(code) = &result {
        ...
        let safe = if code.starts_with("lfm-") {
            code.as_str()
        } else {
            "lfm-persistence-failed"
        };
...
pub(crate) async fn speak_lfm_reply(
    ...
    let presentation =
        crate::voice_behavior::effective_presentation(&state, None, &conversation_id)?;
    ...
    state
        .streaming_tts
        .begin(&state, &run, true, on_event, Some(&conversation_id))
        .await
        .map_err(|_| "lfm-speech-prepare-failed")?;
    if crate::voice_behavior::effective_presentation(&state, None, &conversation_id)?.decision
        != "speak"
    {
        state.streaming_tts.cancel(&run);
        return Ok(());
    }
    let queued = speech_priority::queue_reply(
        &state.streaming_tts,
        &conversation_id,
        &run,
        &text,
    ...
    record(
        ...
        if queued {
            ...
            "lfm-speech-preempted-by-answer"
        },
        None,
    );
    Ok(())
