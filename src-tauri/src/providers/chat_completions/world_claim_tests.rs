#![cfg(test)]
use super::*;
use crate::runtime::context::world::{state_claim::ClaimEvents, wire_test_support::Harness};
...
async fn wr_t18_real_wire_claims_are_validated_before_display_and_changed_sources_rejected() {
    for mode in ["valid", "forged", "changed"] {
        ...
        let sink = tauri::ipc::Channel::<RuntimeEvent>::new(move |body| {
            if format!("{body:?}").contains("delta") {
        ...
        let answer = h
            ...
            .as_ref()
            .unwrap()
            .accept_claims(&raw.unwrap(), "次の期限は？");
        assert_eq!(answer.is_ok(), mode == "valid");
        if let Ok(answer) = answer {
            assert!(answer.contains("claim-deadline"));
            assert!(!answer.contains("claims"));
        }
    }
}
