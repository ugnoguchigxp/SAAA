            ...
            for index in 0..4 {
                ...
                match index {
                    0 => write_response(
                        ...
                        "200 OK",
                        "application/json",
                        &serde_json::from_str::<serde_json::Value>(&saaa_selector_catalog()).unwrap()
                        .to_string(),
                    ),
        ...
        let server = std::thread::spawn(move || {
            for index in 0..6 {
                ...
                let body = match index {
                    ...
                    4 => format!(
                        "data: {}\n\ndata: [DONE]\n\n",
                        json!({"model": LLM_MODEL, "choices":[{"index":0,"delta":{"content":"HTTP claim works"},"finish_reason":"stop"}]})
                    ),
                    5 => String::new(),
                    _ => unreachable!(),
                };
...
fn state_provider(name: &str) -> Value {
    json!({
        ...
        "publicModel": if name == "llm" { LLM_MODEL } else { "other-model" },
        "readiness": "ready",
        "claimable": true
    })
}
