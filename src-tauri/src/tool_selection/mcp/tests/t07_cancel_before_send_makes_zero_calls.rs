use super::*;
#[tokio::test]
pub(super) async fn t07_cancel_before_send_makes_zero_calls() {
...
#[tokio::test]
pub(super) async fn t08_large_result_is_paged_and_reassembles_exactly() {
    let state = default_state();
...
#[tokio::test]
pub(super) async fn t08_result_scope_ttl_and_acl_are_enforced() {
    let state = default_state();
    *state.call_result.lock().unwrap() = json!({
...
#[tokio::test]
pub(super) async fn t08_size_limit_keeps_the_scan_bounded() {
    let state = default_state();
    *state.call_result.lock().unwrap() = json!({
...
#[tokio::test]
pub(super) async fn t09_stale_source_is_not_eligible() {
    let state = default_state();
    let server = MockServer::start(state.clone()).await;
    let harness = Harness::new(&server, vec![user_grant("search")]);
    harness.manager.sync_source("mcp-test").await.expect("sync");
...
#[tokio::test]
pub(super) async fn t10_same_name_in_two_sources_makes_a_name_only_correction_ambiguous() {
...
#[tokio::test]
pub(super) async fn a01_two_sources_with_1500_tools_publish_and_search_is_bounded() {
...
use super::super::repository as mcp_repo;
