...
pub(crate) const PRINCIPAL: &str = "P1";
const CONVERSATION: &str = "conversation_primary";
const DIM: usize = 8;
pub(crate) const MODEL_HASH: &str = "test-constant";
...
pub(crate) struct ConstantEmbedding;
#[async_trait]
...
pub(crate) struct BarrierBackend {
...
pub(crate) struct MutatingReranker {
    pub(crate) inner: FixedReranker,
    pub(crate) writer: Arc<SqliteWriter>,
    pub(crate) calls: AtomicUsize,
    pub(crate) mutated: Mutex<bool>,
}
impl MutatingReranker {
    pub(crate) fn new(writer: Arc<SqliteWriter>, scores: &[(&str, f64)]) -> Self {
        Self {
...
pub(crate) struct Harness {
    pub(crate) writer: Arc<SqliteWriter>,
    pub(crate) backend: Arc<FixtureBackend>,
}
impl Harness {
    ...
    pub(crate) fn service(&self, extractor_body: &str) -> ToolSelectionService {
    ...
    pub(crate) fn service_with_backend(
        &self,
        backend: Arc<dyn super::super::backends::ToolBackend>,
        extractor_body: &str,
    ...
    pub(crate) fn service_with_reranker(
        &self,
    ...
    pub(crate) fn register(&self, spec: &ToolSpec) -> String {
    ...
    pub(crate) fn register_pair(&self) -> (String, String, String) {
    ...
    pub(crate) fn grant_user(&self, principal: &str, tool_id: &str) {
    ...
    pub(crate) fn insert_message(&self) -> String {
    ...
    pub(crate) fn insert_conversation(&self, id: &str) {
        let id = id.to_string();
        let now = crate::now_iso();
        write_tx(&self.writer, move |connection| {
            connection
    ...
    pub(crate) fn context(&self, message: Option<String>, project: Option<&str>) -> RequestContext {
...
pub(crate) fn vector(first: f32) -> Vec<f32> {
...
pub(crate) fn order(response: &super::super::service::SearchResponse) -> Vec<String> {
    response
        .candidates
        .iter()
...
pub(crate) const USER_MESSAGE: &str = "この案件の過去の判断はWebでなく議事録で";
pub(crate) const NO_FEEDBACK: &str = r#"{"scenario":{"intent":"案件の過去の判断を確認","operation":"search","objectType":"decision_record","phase":"discover","inputKind":"text"},"feedback":[]}"#;
#[tokio::test]
pub(crate) async fn g01_basic_order_is_reranker_order() {
...
#[tokio::test]
pub(crate) async fn ai_09_synthetic_approved_artifact_reorders_the_real_tool_search_path() {
    let harness = Harness::new();
...
#[tokio::test]
pub(crate) async fn g02_project_correction_applies_to_the_next_search() {
    let harness = Harness::new();
    harness.register_pair();
    let message = harness.insert_message();
    let context = harness.context(Some(message), Some("A"));
