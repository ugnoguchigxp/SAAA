...
#[tokio::test]
pub(crate) async fn revoke_of_an_unresolvable_tool_keeps_existing_rules() {
...
#[tokio::test]
pub(crate) async fn startup_reconcile_settles_running_invocations() {
    let harness = Harness::new();
    harness.register_pair();
...
#[tokio::test]
pub(crate) async fn explicit_preference_injects_a_tool_retrieval_missed() {
    let harness = Harness::new();
...
#[tokio::test]
pub(crate) async fn missing_embedding_index_is_reported_as_degraded_not_no_match() {
    let harness = Harness::new();
    harness.register_pair();
    harness
        .writer
...
#[tokio::test]
pub(crate) async fn gateway_rejects_wrong_typed_describe_fields() {
    let harness = Harness::new();
    harness.register_pair();
    let message = harness.insert_message();
    let context = harness.context(Some(message), Some("A"));
...
struct MixedBackend {
    ...
    pub(crate) calls: Mutex<Vec<BackendRequest>>,
}
impl MixedBackend {
    fn new() -> Self {
        Self {
...
impl ToolBackend for MixedBackend {
    async fn invoke(
        ...
        match self.counter.fetch_add(1, Ordering::SeqCst) % 3 {
            ...
            _ => BackendOutcome {
                status: super::super::backends::TechnicalStatus::Unknown,
                result: None,
...
#[tokio::test]
pub(crate) async fn p03_100_mixed_calls_never_invent_positive_satisfaction() {
    let harness = Harness::new();
    harness.register_pair();
    let message = harness.insert_message();
    let context = harness.context(Some(message), Some("A"));
