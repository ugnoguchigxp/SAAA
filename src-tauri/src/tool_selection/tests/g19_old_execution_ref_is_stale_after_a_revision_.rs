...
#[tokio::test]
pub(super) async fn g19_old_execution_ref_is_stale_after_a_revision_change() {
...
#[tokio::test]
pub(super) async fn g20_unavailable_reranker_degrades_without_faking_confidence() {
    let harness = Harness::new();
    harness.register_pair();
    let message = harness.insert_message();
    let context = harness.context(Some(message), Some("A"));
...
#[tokio::test]
pub(super) async fn describe_usage_pages_are_paginated_and_bound() {
    let harness = Harness::new();
    harness.register_pair();
    let message = harness.insert_message();
    let context = harness.context(Some(message), Some("A"));
...
#[tokio::test]
pub(super) async fn search_response_contract_is_bounded() {
    let harness = Harness::new();
    harness.register_pair();
    let message = harness.insert_message();
    let context = harness.context(Some(message), Some("A"));
...
#[tokio::test]
pub(super) async fn gateway_search_describe_invoke_round_trip() {
    let harness = Harness::new();
    harness.register_pair();
    let message = harness.insert_message();
    let context = harness.context(Some(message), Some("A"));
    ...
    let search = super::super::gateway::dispatch(
        &service,
        &context,
        super::super::gateway::TOOL_SEARCH,
    ...
    let describe = super::super::gateway::dispatch(
        &service,
        &context,
        super::super::gateway::TOOL_DESCRIBE,
    ...
    let invoke = super::super::gateway::dispatch(
        &service,
        &context,
        super::super::gateway::TOOL_INVOKE,
...
#[tokio::test]
pub(super) async fn gateway_rejects_unknown_arguments_without_leaking_details() {
    let harness = Harness::new();
    harness.register_pair();
    let message = harness.insert_message();
    let context = harness.context(Some(message), Some("A"));
    let service = harness.service(NO_FEEDBACK);
    let cancellation = crate::RunCancellation::default();
    let response = super::super::gateway::dispatch(
        &service,
        &context,
        super::super::gateway::TOOL_SEARCH,
...
pub(super) async fn mock_json_provider(body: String) -> (String, tokio::task::JoinHandle<()>) {
...
#[tokio::test]
pub(super) async fn provider_extraction_uses_the_configured_conversation_provider() {
    let harness = Harness::new();
    harness.register_pair();
    let extraction = json!({
        "scenario": {
...
pub(super) async fn e01_real_llang_invoke_true_and_false_through_selection() {
    ...
    let backend = Arc::new(super::super::backends::llang::LlangBackend::new(Some(
        env.service.clone(),
    )));
