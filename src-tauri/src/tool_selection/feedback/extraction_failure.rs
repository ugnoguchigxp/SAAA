...
pub(super) fn parse_evidence(value: &Value, user_message: &str) -> Option<Evidence> {
    let object = value.as_object()?;
...
pub(super) fn parse_condition(value: Option<&Value>) -> FeedbackCondition {
...
pub(super) fn salvage_feedback(value: &Value) -> Option<ExtractedFeedback> {
    let object = value.as_object()?;
...
pub(super) fn resolved_condition(
    feedback: &ExtractedFeedback,
    scenario: &Scenario,
