...
pub fn migrate(connection: &Connection) -> rusqlite::Result<()> {
    ...
    super::mcp::schema::migrate(connection)
}

/// Delegates the version-24 `tool_selection_sources.kind` rebuild to the MCP schema module.
pub fn migrate_sources_kind(connection: &Connection) -> rusqlite::Result<()> {
    super::mcp::schema::migrate_sources_kind(connection)
}
