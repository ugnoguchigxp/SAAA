    ...
    pub(super) fn current_revision(
        ...
        let result = self
            ...
            .read_serialized(move |connection| {
                ...
                if !super::super::source_lookup::source_eligible(connection, &tool.source_id, now)
                    .map_err(|error| error.to_string())?
                {
                    return Err("stale".to_string());
                }
    ...
    pub async fn invoke_with_origin(
        ...
        if binding_kind == "mcp_http" {
            ...
            manager
                ...
                .await
                .map_err(super::super::mcp::service_support::map_call_error)?;
        }
...
impl ToolSelectionService {
    ...
    pub fn describe_result(
        ...
        super::super::mcp::service_support::describe_result(&self.writer, context, result_ref, page)
    }
}
impl ToolSelectionService {
...
pub(super) const CHANGED: &str = "selection-changed";
