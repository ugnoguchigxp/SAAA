...
pub(crate) fn publish(
    ...
    super::super::driver::consume(connection)?;
...
pub(crate) fn queue_terminals(
    ...
    if !speech_holds_tts(state) {
        super::super::outbox::flush_unflushed(connection, conversation_id, now)?;
    }
    Ok(())
}
pub(crate) fn flush_held_reports(state: &AppState, conversation_id: &str) -> Result<(), String> {
    ...
    super::super::reduce::start_queued_for_conversation(state, conversation_id)?;
...
pub(super) fn start_pending_speech(state: &AppState, conversation_id: &str) -> Result<(), String> {
...
pub(super) fn speech_event_state(body: &tauri::ipc::InvokeResponseBody) -> Option<&'static str> {
...
pub(super) fn record_delivered_notification_outcomes(
    connection: &Connection,
    conversation_id: &str,
    now_ms: i64,
    message_id: &str,
...
pub(super) fn notification_selection(
    connection: &Connection,
    goal_id: &str,
    requested: &str,
    now: i64,
