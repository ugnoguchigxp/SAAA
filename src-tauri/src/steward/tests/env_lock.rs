...
pub(crate) fn with_memory(on: bool, run: impl FnOnce()) {
    let _guard = env_lock()
        .lock()
...
pub(crate) fn workspace(connection: &Connection) -> String {
    connection
        .execute(
...
pub(crate) fn register_goal(state: &AppState) {
    state
        .sqlite_writer
        .write(|connection| {
...
pub(crate) fn count(state: &AppState, sql: &str) -> i64 {
    state
        .sqlite_readers
        .read(|connection| {
            connection
...
pub(crate) fn insert_job(state: &AppState, task_id: &str, job_state: &str, identity: Option<&str>) {
    state
        .sqlite_writer
        .write(|connection| {
            ...
            super::super::evidence::persist(
                connection,
                &super::super::evidence::from_host_session(
                    task_id,
                    "job",
                    "run",
                    job_state,
                    ...
                    Some("digest"),
                    Some(0),
                    super::super::evidence::PRODUCER_HOST_RECIPE,
                ),
            )?;
...
pub(crate) fn task_id(state: &AppState) -> String {
    state
        .sqlite_readers
        .read(|connection| {
            connection
...
#[test]
pub(crate) fn ml_00_filter_runs() {
    assert!(!START_TRIGGER.is_empty());
}
#[test]
pub(crate) fn ml_01_schema_version_and_empty_goals() {
    let connection = db();
    let version: i64 = connection
        .pragma_query_value(None, "user_version", |row| row.get(0))
        .expect("version");
    assert_eq!(version, DATABASE_SCHEMA_VERSION);
    assert_eq!(DATABASE_SCHEMA_VERSION, 40);
...
#[test]
pub(crate) fn ml_01_status_changes_insert_not_overwrite() {
...
#[test]
pub(crate) fn ml_01_invalid_ops_rejected() {
    let connection = db();
...
#[test]
pub(crate) fn dw_04_direct_registration_persists_only_the_confirmed_bounded_scope() {
    let connection = db();
    workspace(&connection);
...
#[test]
pub(crate) fn ml_02_register_requires_workspace() {
    let state = app_state(db());
...
#[test]
pub(crate) fn dw_01_multiple_active_goals_are_allowed_and_no_delete() {
    let state = app_state(db());
    register_goal(&state);
    let second = state
        .sqlite_writer
...
#[test]
pub(crate) fn dw_01_direct_registration_allows_eight_goals_then_enforces_the_limit() {
    let connection = db();
    workspace(&connection);
    for index in 0..8 {
...
#[test]
pub(crate) fn dw_01_proposal_rejects_ambiguous_or_unbounded_authority() {
...
#[test]
pub(crate) fn dw_01_plan_rejects_cycles_and_replan_limit() {
    let plan = TaskPlan {
        steps: vec![PlanStep {
            id: "inspect".into(),
            depends_on: Vec::new(),
...
#[test]
pub(crate) fn dw_10_settled_read_step_durably_enqueues_one_dependent_test_step() {
    with_memory(true, || {
        let state = app_state(db());
        register_goal(&state);
...
#[test]
pub(crate) fn dw_10_migration_backfills_a_plan_for_an_existing_goal() {
    let connection = db();
    workspace(&connection);
