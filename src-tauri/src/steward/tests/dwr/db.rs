...
pub(crate) fn proposal(source: &str, summary: &str) -> GoalProposal {
    GoalProposal {
...
pub(crate) fn workspace(connection: &Connection) {
    connection
        .execute(
...
#[test]
pub(crate) fn dw_r02_contract_rejects_stale_or_ambiguous_bindings() {
    let text = "日本語テスト😀終端";
...
#[test]
pub(crate) fn dw_r02_plan_rejects_duplicate_step_ids() {
    let plan = TaskPlan {
        steps: vec![
            PlanStep {
                id: "read".into(),
...
#[test]
pub(crate) fn dw_r02_migration_preserves_all_lineage_and_constraints() {
...
#[test]
pub(crate) fn dw_r03_quoted_negative_and_tool_sources_never_grant_authority() {
...
#[test]
pub(crate) fn dw_r03_existing_grant_needs_no_repeat_confirmation() {
    let state = app_state(db());
    register(&state);
...
#[test]
pub(crate) fn dw_r03_stale_confirmation_is_rejected() {
    let state = app_state(db());
    state
        .sqlite_writer
        .write(|c| {
...
#[test]
pub(crate) fn dw_r04_proposal_creates_plan_task_reservation_and_intent_atomically() {
    let state = app_state(db());
    register(&state);
...
#[test]
pub(crate) fn dw_r04_duplicate_at_goal_limit_returns_original_receipt() {
    let state = app_state(db());
    state
        .sqlite_writer
        .write(|c| {
...
#[test]
pub(crate) fn dw_r04_admission_fault_rolls_back_every_row() {
    let state = app_state(db());
    state
        .sqlite_writer
        .write(|c| {
...
#[test]
pub(crate) fn dw_r05_withdraw_either_goal_preserves_the_other() {
    let state = app_state(db());
...
#[test]
pub(crate) fn dw_r05_late_result_and_status_refresh_cannot_revive_work() {
    let state = app_state(db());
    register(&state);
...
#[test]
pub(crate) fn dw_r06_profile_gate_reads_writer_snapshot_without_reentering_readers() {
    let state = app_state(db());
    state
        .sqlite_writer
        .write(|connection| {
...
#[test]
pub(crate) fn dw_r06_revoke_before_dispatch_creates_no_job() {
    let state = app_state(db());
...
#[test]
pub(crate) fn dw_r07_unknown_never_resends() {
    let state = app_state(db());
    register(&state);
...
#[test]
pub(crate) fn dw_r09_read_boundary_rejects_write_network_shell_and_escape() {
...
#[test]
pub(crate) fn dw_r10_registered_recipe_produces_pass_and_fail_evidence() {
    let connection = db();
...
#[test]
pub(crate) fn dw_r10_model_cannot_change_recipe_argv_or_output_scope() {
    let connection = db();
...
#[test]
pub(crate) fn dw_r12_verifier_distinguishes_report_pass_failure_and_missing_evidence() {
    assert_eq!(
...
#[test]
pub(crate) fn dw_r19_registered_goal_without_tasks_is_visible_and_withdrawable() {
    let state = app_state(db());
...
#[test]
pub(crate) fn dw_r19_queries_never_dispatch_or_mutate_delivery() {
    let state = app_state(db());
    register(&state);
...
#[test]
pub(crate) fn dw_r11_restart_and_busy_do_not_reset_or_double_charge_budget() {
    let connection = db();
...
#[test]
pub(crate) fn dw_r13_failed_unknown_and_awaiting_user_do_not_unlock_success_dependencies() {
    let plan = TaskPlan {
        steps: vec![
            PlanStep {
                id: "read".into(),
