use super::*;
#[test]
pub(super) fn dw_r14_forget_during_hold_or_pending_speech_prevents_redisplay() {
    let connection = db();
    connection
        .execute(
...
#[test]
pub(super) fn dw_r15_claim_failure_never_loses_report() {
    let connection = db();
...
#[test]
pub(super) fn dw_r16_lost_wakeup_is_recovered_from_database() {
    let state = app_state(db());
    crate::steward::pump::drain(&state).unwrap();
}
#[test]
pub(super) fn dw_r18_reports_show_evidence_and_actionable_unknown() {
...
#[test]
pub(super) fn dw_r22_full_flow_without_ui_or_new_turn() {
    let state = app_state(db());
    register(&state);
    crate::steward::pump::drain(&state).unwrap();
...
#[test]
pub(super) fn dw_r06_schedule_dispatch_shares_commit_path_and_gates() {
    let state = app_state(db());
    register(&state);
...
#[test]
pub(super) fn dw_r12_tests_pass_cannot_be_approved_without_evidence() {
    assert_eq!(
...
pub(super) fn register_named(state: &AppState, summary: &str) -> String {
    state
        .sqlite_writer
        .write(|c| {
            let _ = c.execute(
...
pub(super) fn count(state: &AppState, sql: &str) -> i64 {
    state
        .sqlite_readers
        .read(|c| {
...
pub(super) fn queue_one(state: &AppState, run: &str, content: &str) -> String {
