use super::*;
#[test]
pub(super) fn ml_05_hold_skips_insert_then_flush_one() {
    with_memory(true, || {
        ...
        state
            .sqlite_writer
            .write(|connection| super::super::report::publish(&state, connection, PRIMARY_CONVERSATION_ID))
            .unwrap();
        assert_eq!(
            count(
                &state,
...
#[test]
pub(super) fn dw_14_multiple_goals_keep_the_sibling_through_topic_switch_withdrawal_and_hold() {
    with_memory(true, || {
        let state = app_state(db());
        state
            .sqlite_writer
...
#[test]
pub(super) fn ml_06_withdraw_blocks_start_and_continue() {
    with_memory(true, || {
        let state = app_state(db());
        register_goal(&state);
...
#[test]
pub(super) fn ml_06_withdraw_does_not_rewrite_done() {
    with_memory(true, || {
        let state = app_state(db());
        register_goal(&state);
...
#[test]
pub(super) fn ml_07_reopen_maps_outcome_unknown_without_rerun() {
...
#[test]
pub(super) fn ml_08_acceptance_register_divert_complete_withdraw() {
    with_memory(true, || {
        let state = app_state(db());
        register_goal(&state);
...
#[test]
pub(super) fn ui_queue_reorder_is_durable_and_changes_dispatch_priority() {
    let connection = db();
    connection
