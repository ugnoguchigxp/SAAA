...
pub(crate) fn register_with_options(
    ...
    let active = super::super::admission::incomplete_goal_count(connection, conversation_id)?;
...
pub(super) fn persist_default_goal_plan(
    connection: &Connection,
    goal_id: &str,
    ops: &str,
    verifier: &str,
...
pub(crate) fn propose(
    ...
    let result = super::super::intake::classify(
        connection,
        conversation_id,
        &proposal.source_message_id,
        proposal,
    )?;
    Ok(super::super::admission::json_result(result))
}
