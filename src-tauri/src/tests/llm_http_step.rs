...
pub(super) async fn spawn_llm_http_fixture(
    steps: Vec<LlmHttpStep>,
) -> (String, Arc<Mutex<Vec<String>>>, tokio::task::JoinHandle<()>) {
    spawn_llm_http_sequence_fixture(vec![steps]).await
}
pub(super) async fn spawn_llm_http_sequence_fixture(
...
pub(super) fn begin_test_provider_session(
    state: &AppState,
    runtime_run_id: &str,
    provider_id: &str,
    provider_kind: &str,
...
#[test]
pub(super) fn duplicate_run_registration_preserves_the_original_cancellation_handle() {
...
#[test]
pub(super) fn app_shutdown_cancels_every_active_run_and_resets_situation_to_safe_state() {
    let connection = Connection::open_in_memory().expect("database opens");
    initialize_database(&connection).expect("database initializes");
    let state = app_state(connection);
...
#[test]
pub(super) fn runtime_and_provider_session_finalization_is_one_shot() {
    let connection = Connection::open_in_memory().expect("database opens");
    initialize_database(&connection).expect("database initializes");
    connection
        .execute(
...
#[test]
pub(super) fn dynamic_lan_session_persists_release_success_and_deferred_cleanup() {
    let connection = Connection::open_in_memory().expect("database opens");
    initialize_database(&connection).expect("database initializes");
    connection
        .execute(
...
#[test]
pub(super) fn dynamic_lan_cleanup_keeps_release_debt_across_connection_replacement() {
...
#[tokio::test]
pub(super) async fn cancellation_notification_remains_observable_for_late_waiters() {
...
#[test]
pub(super) fn runtime_and_voice_events_serialize_camel_case_fields() {
...
#[test]
pub(super) fn foreign_message_flood_cannot_starve_a_request_deadline() {
...
#[test]
pub(super) fn normal_turns_reject_legacy_conversation_ids_without_writing_partial_state() {
    let connection = Connection::open_in_memory().expect("database opens");
    initialize_database(&connection).expect("database initializes");
    connection
        .execute(
...
#[test]
pub(super) fn task_specific_workspace_validation_precedes_runtime_writes() {
    let connection = Connection::open_in_memory().expect("database opens");
    initialize_database(&connection).expect("database initializes");
    connection
        .execute(
...
#[test]
pub(super) fn normal_turns_reject_byte_oversized_context_before_writing_partial_state() {
    let connection = Connection::open_in_memory().expect("database opens");
    initialize_database(&connection).expect("database initializes");
    let state = app_state(connection);
...
#[test]
pub(super) fn provider_transport_ledger_records_phases_without_payloads() {
    let connection = Connection::open_in_memory().expect("database opens");
    initialize_database(&connection).expect("database initializes");
    connection
        .execute(
