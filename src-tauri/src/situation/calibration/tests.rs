...
fn v1_fixture_defaults_input_activity_without_changing_the_scene_baseline() {
    ...
    assert!(samples.iter().all(|sample| {
        sample.signals.input_activity.state == super::super::contracts::InputActivityState::Unknown
...
fn accept_and_rollback_require_the_exact_profile_lifecycle() {
    ...
    let candidate =
        create_candidate(&connection, CalibrationParameters::default()).expect("candidate creates");
