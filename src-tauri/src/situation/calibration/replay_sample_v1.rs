...
pub(super) struct ReplaySampleV1 {
    pub(super) elapsed_ms: u64,
    pub(super) signals: super::super::contracts::SignalSnapshot,
    pub(super) expected_scene: SituationScene,
}
#[derive(Deserialize)]
#[serde(rename_all = "camelCase", deny_unknown_fields)]
...
struct ReplayScenario {
    pub(super) id: String,
    pub(super) samples: Vec<ReplaySampleV2>,
}
#[derive(Deserialize)]
#[serde(rename_all = "camelCase", deny_unknown_fields)]
struct ReplaySampleV2 {
    ...
    pub(super) signals: super::super::contracts::SignalSnapshot,
    pub(super) expected_scene: SituationScene,
    pub(super) expected_attention: String,
}
...
pub(super) fn load_v1_samples() -> Result<Vec<ReplaySampleV1>, String> {
    serde_json::from_str(include_str!("../../../fixtures/situation/mvp1-v1.json"))
        .map_err(|_| "Invalid MVP 1 replay fixture".to_string())
}
pub(super) fn load_v2_fixture() -> Result<ReplayFixtureSet, String> {
...
pub(super) struct ReplaySummary {
    ...
    pub(super) expected_scene_matches: u64,
    pub(super) expected_attention_samples: u64,
    pub(super) expected_attention_matches: u64,
    pub(super) policy_counts: [u64; 4],
}
...
pub(super) fn replay_scenario<'a>(
    samples: impl Iterator<
        Item = (
            u64,
            &'a super::super::contracts::SignalSnapshot,
            SituationScene,
            Option<&'a str>,
        ),
    >,
    ...
    for (elapsed_ms, signals, expected_scene, expected_attention) in samples {
        ...
        let candidate = super::super::classifier::classify_with_parameters(signals, parameters);
...
pub(super) fn profile_row(row: &rusqlite::Row<'_>) -> rusqlite::Result<CalibrationProfile> {
...
pub(super) fn validate_profile(profile: &CalibrationProfile) -> Result<(), String> {
...
pub(super) fn validate_run(run: &CalibrationRun) -> Result<(), String> {
...
pub(super) fn validate_rule_version(version: &str) -> Result<(), String> {
    if version.is_empty()
        || version.len() > 160
...
pub(super) fn validate_decision_reason(reason: &str) -> Result<(), String> {
    if matches!(
        reason,
        "wrong-scene"
            | "stale-signal"
