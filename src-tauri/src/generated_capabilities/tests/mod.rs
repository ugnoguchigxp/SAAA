...
impl TestEnv {
    ...
    pub(crate) async fn verify(
        ...
        acceptance_id: &str,
    ) -> CapabilityResult<super::service::VerificationSummary> {
        self.service
            .verify_candidate(
...
#[test]
fn w01_real_v2_responses_parse_strictly() {
...
#[test]
fn w02_protocol_id_and_hash_correlation_is_enforced() {
...
#[test]
fn w03_string_boolean_is_not_a_success_value() {
...
#[test]
fn w04_outer_ok_does_not_hide_inner_failures() {
...
#[test]
fn w05_unknown_fields_versions_and_empty_cases_are_rejected() {
...
async fn h01_both_candidates_run_on_one_runtime_digest() {
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn h02_tampered_runtime_and_missing_bun_fail_before_spawn() {
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn h03_hanging_host_is_killed_reaped_and_replaced() {
    let env = TestEnv::start(false);
    let host = fixture_host(&env);
    let temporary = tempfile::tempdir().unwrap();
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn h04_child_environment_is_cleared() {
    let temporary = tempfile::tempdir().unwrap();
    let command = script_command(
        temporary.path(),
        "env",
    ...
    let error = super::host::process::execute(
        &command,
        b"{}",
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn feature_flag_off_refuses_management_and_execution() {
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn m0r_runtime_is_reusable_from_a_moved_directory_with_spaces() {
    let temporary = tempfile::tempdir().unwrap();
