use super::*;
pub(super) fn hash64() -> String {
    "a".repeat(64)
}
pub(super) fn actor() -> CallActor {
    CallActor {
...
pub(super) fn capability_id(env: &TestEnv, revision_id: &str) -> String {
    env.writer
...
pub(super) fn job_status(env: &TestEnv, job_id: &str) -> String {
    env.writer
        .read_serialized(|connection| {
            connection
                .query_row(
...
pub(super) fn grants(env: &TestEnv) -> i64 {
    env.scalar("SELECT COUNT(*) FROM tool_selection_grants")
}
pub(super) fn catalog_enabled(env: &TestEnv) -> i64 {
    env.scalar("SELECT COUNT(*) FROM tool_selection_catalog WHERE enabled = 1")
}
pub(super) async fn generate_a(env: &TestEnv) -> (GenerationService, Arc<FakeGenerator>, String) {
...
pub(super) async fn invoke_on(env: &TestEnv, call_id: &str, revision_id: &str) -> String {
...
pub(super) struct GatedGenerator {
    pub(super) inner: FakeGenerator,
...
#[tokio::test]
pub(super) async fn gc_01_fixture_inspector_stores_display_without_a_live_kit() {
...
#[tokio::test]
pub(super) async fn gc_02_inspect_after_b_keeps_revision_a_typescript() {
    let env = TestEnv::start(true);
...
#[tokio::test]
pub(super) async fn gc_03_wrong_acceptance_keeps_a_and_does_not_grant() {
    let env = TestEnv::start(true);
    let request_a = registered("req-a", CANDIDATE_A, ACCEPTANCE_A, true, false);
...
#[tokio::test]
pub(super) async fn gc_04_suspend_unpublishes_catalog_and_keeps_past_inspect() {
    let env = TestEnv::start(true);
...
#[tokio::test]
pub(super) async fn gc_05_cancel_and_timeout_finish_the_job() {
    let env = TestEnv::start(true);
    let request_a = registered("req-a", CANDIDATE_A, ACCEPTANCE_A, true, false);
