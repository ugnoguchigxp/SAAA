...
pub(super) fn ensure_unique<'a>(values: impl Iterator<Item = &'a str>, label: &str) -> Result<(), String> {
...
pub(super) fn is_field_name(value: &str) -> bool {
    let mut bytes = value.bytes();
    match bytes.next() {
...
pub(super) fn write_canonical(value: &Value, out: &mut String) {
    match value {
