...
impl Harness {
    pub(crate) fn new() -> Self {
        ...
        let history = composed
            ...
            .iter()
            .enumerate()
            .map(|(i, m)| crate::ConversationMessage {
                id: format!("context-{i}"),
    ...
    pub(crate) fn assert_wire(&self, body: &serde_json::Value) {
        let content = body["messages"]
            ...
            .find(|s| s.contains(super::render::WORLD_HEADER))
            .unwrap();
        let json = content
            .split_once(super::render::WORLD_HEADER)
            .unwrap()
            .1
            .split_once(super::render::WORLD_FOOTER)
            .unwrap()
            .0
            .trim();
...
pub(crate) const TRANSITIONS: [&str; 7] = [
    "initial",
    "tool-continuation",
    "fallback",
    "scope-switch",
