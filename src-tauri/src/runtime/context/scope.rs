...
fn ensure_scope(connection: &Connection, kind: &str, id: &str) -> Result<String, String> {
    let key = format!("{kind}:{id}");
    connection
        .execute(
...
fn epoch(connection: &Connection, key: &str) -> Result<u64, String> {
    connection
        .query_row(
...
fn bump_epoch(connection: &Connection, key: &str) -> Result<(), String> {
    connection
        .execute(
...
fn linked(connection: &Connection, left: &str, right: &str) -> bool {
    connection
        .query_row(
...
fn snapshot_digest(status: &str, focus: Option<&str>, scopes: &[ResolvedScope]) -> String {
