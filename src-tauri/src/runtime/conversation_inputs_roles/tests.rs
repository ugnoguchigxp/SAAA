...
fn accept_receipt(connection: &mut Connection, run_id: &str, now_ms: i64) {
    ...
    let input =
        crate::role_routing::availability::selection_input_for(&policy, &unknown_reachability());
    assert!(
...
fn codex_actor_selects_the_isolated_dispatch_without_rewriting_provider_settings() {
    ...
    let dispatch = apply_enabled_role_route(
        &connection,
        Some("codex-run"),
        &mut route,
...
fn rr_22_role_route_enforces_the_step_budget() {
    ...
    connection
            ...
            .expect("spent step");
    assert!(apply_enabled_role_route(
        &connection,
        Some("budget-run"),
        &mut route,
        &unknown_reachability()
...
fn rr_ls_20_unreachable_selects_the_away_recipe() {
    ...
    let selected: String = connection
        .query_row(
            "SELECT selected_id FROM rr_decisions WHERE root_id='away-run'",
            [],
            |row| row.get(0),
        )
        .expect("selected");
    let reasons: String = connection
        .query_row(
            "SELECT reason_codes_json FROM rr_decisions WHERE root_id='away-run'",
            [],
            |row| row.get(0),
        )
        .expect("reasons");
    let actor: String = connection
        .query_row(
            "SELECT actor_id FROM rr_steps WHERE root_id='away-run'",
            [],
            |row| row.get(0),
        )
        .expect("actor");
...
fn rr_ls_21_reachable_selects_the_home_recipe() {
    ...
    let selected: String = connection
        .query_row(
            "SELECT selected_id FROM rr_decisions WHERE root_id='home-run'",
            [],
            |row| row.get(0),
        )
        .expect("selected");
    let actor: String = connection
        .query_row(
            "SELECT actor_id FROM rr_steps WHERE root_id='home-run'",
            [],
            |row| row.get(0),
        )
        .expect("actor");
...
fn rr_ls_22_dispatch_refuses_a_lan_actor_that_became_unreachable() {
    ...
    assert_eq!(
        error,
        "Role-routing LAN provider is unreachable before dispatch"
    );
    assert_eq!(
        connection
            .query_row("SELECT count(*) FROM provider_sessions", [], |row| row
