...
pub(crate) fn start(
    ...
    Some(tauri::async_runtime::spawn(async move {
        ...
        let canonical = if language == "en" {
            "I received the request and am thinking about the answer."
        } else {
            "依頼を受け取りました。回答を考えています。"
        };
        speak(
            &conversation_id,
            &run_id,
        ...
        let canonical = if language == "en" {
            "I am still working through the request."
        } else {
            "引き続き依頼内容を確認し、回答を考えています。"
        };
        speak(
            &conversation_id,
            &run_id,
            crate::larm_voice::ResponseKind::Thinking,
            canonical,
            &language,
            &*events,
            cancellation,
