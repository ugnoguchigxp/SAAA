...
pub(super) const MAX_CANDIDATES: usize = 1_000;
...
pub(super) fn is_challenge(failure: &WebFetchFailure) -> bool {
...
pub(super) fn can_try_alternate(failure: &WebFetchFailure) -> bool {
    matches!(
        failure.code,
...
pub(super) fn extract_preload_url(html: &str) -> Result<String, WebFetchFailure> {
    let parse_changed = || {
        WebFetchFailure::new(
            "PARSE_CHANGED",
            "The search provider changed its response format.",
...
pub(super) fn extract_vqd(html: &str) -> Option<String> {
    let bytes = html.as_bytes();
    let mut cursor = 0;
...
pub(super) fn parse_ddg_web(body: &str, limit: usize) -> Result<Vec<RawHit>, WebFetchFailure> {
    let parse_changed = || {
        WebFetchFailure::new(
            "PARSE_CHANGED",
            "The search provider changed its response format.",
