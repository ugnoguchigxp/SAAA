...
更新日: 2026-09-21。offline実装残件は完了。実Providerの製品受入とは区別する。
...
| T08–T10 | strict候補schema、専用purpose、既存workerのlease/cancel、host根拠付与、訂正・冪等commit | 五要素併用、訂正、replay、forgetを含め合格 |
...
| T15 | Codex app-serverとrole SDKの共通Frame、最終wire receipt、現在依頼の二重投入防止 | app-serverの全遷移とrole SDK最終JSONL observer fixture合格 |
| T16 | reasoning-answer-v3 / world-evidence-v2、初期化後refresh、実RPCとschema一致 | schema・遅延初期化fixture合格 |
| T17–T18 | source/digest/value/as-ofの一致、質問対象検査、次の期限の最小値検査、未検証Delta遮断、保存直前DB再検査 | 正常/偽claim/生成中変更、保存直前の変更拒否まで合格 |
...
| T22 | 既存33ケース維持、カード試験runner、前回report無効化、欠落/重複/0件検知 | 6経路×7遷移、42/42合格。MCP Tool継続だけ能力契約に基づくN/A |
| T23 | module分割、上限100投影/2000ledger/8 Coding/8期限の比較測定 | 追加p95 5.235001ms、30標本。担当module size合格。全体既存違反は製品受入へ残す |
| T24 | 本証跡、親計画、配備手順の更新 | offline実装残件完了。live30例、実UI/音声、TTFAは製品受入へ引継ぎ |
...
- 忘却済みsourceへ依存する五要素assertionはquery時に除外し、再構築後も旧要素を復活させない。
