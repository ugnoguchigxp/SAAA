        ...
        with response:
            ...
            if isinstance(value, dict):
                ...
                if isinstance(error, dict) and isinstance(error.get("message"), str):
                    ...
                    if message.startswith(("upstream returned", "upstream transcription", "unsupported", "unknown", "invalid")) and len(message) <= 250:
            ...
            if record["contentType"] and "audio/wav" in record["contentType"]:
                with wave.open(io.BytesIO(body), "rb") as wav:
                    record["wav"] = {"channels": wav.getnchannels(), "sampleRate": wav.getframerate(), "sampleWidthBytes": wav.getsampwidth(), "frames": wav.getnframes(), "durationSeconds": wav.getnframes() / wav.getframerate()}
                return body
            return value
