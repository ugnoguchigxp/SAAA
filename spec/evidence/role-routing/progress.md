...
| RR-04〜06 | receipt transaction、queue上限拒否、provider route override、pure reducer/ranker断片 | receipt retry/conflict、queue実行、adapter/contextは未実装 |
...
| RR-18 | 起動時にin-flight rootをinterrupted evidenceとして永続化。queued receiptは開始せず順序維持 | queue実行、再接続は未実装 |
...
| RR-19 | 固定SDK版のJSONL sidecarとRust JSONL frame validatorを追加。frame/累積/最終結果の上限とcancelを強制 | bundled binary、SDK process adapter、mock/live wire試験は未実施 |
| RR-20 | sidecar SDKを空の一時cwd、read-only、approval/network/web/MCP無効化、限定環境で起動するよう固定 | bundled resource、fake executable/cancel-tree、live isolation試験は未実施 |
...
- 隔離test binary: `rr_19_frame_validation_rejects_unknown_ids_and_duplicate_terminals`、`rr_19_frame_validation_rejects_extra_fields_and_empty_results`: 各1 pass
