...
| RR-15 | R1 設定 UI と縦通し gate | E23/E27 | E22 | offline pass / live待ち |
...
| RR-38 | tool-specialist 差し替え試験 | E22 | E14/E16 | offline pass |
...
| A02 | E01/E07 | `rr_03_policy_cas_conflict`, `rr_03_policy_validation` | offline | pass（CAS、製品validator、有限recipe compile/依存/独立review検査） |
...
| A07 | E11 | `rr_09_shared_resource_group` | offline | pass（共有resource groupの同時slotを拒否） |
...
| A12 | E27 | `rr_15_asr_tool_tts_reconnect_e2e` | offline | pass（voice source receipt→specialist host tool→親最終回答→永続TTS intent→UI terminal/replay。TTS failure注入後も回答completedを維持） |
| A13 | E05 | `rr_16_multiple_pending_inputs` | offline | pass（E05 実装） |
| A14 | E23 | `rr_16_status_does_not_cancel` | offline | pass |
| A15 | E23/E26 | `rr_16_clarification_keeps_barrier` | offline | pass（`rr_29_provider_before_classifier_raises_durable_barrier_and_queues_follow_up`。通常入力receiptで旧root=`draining`、新root=`queued`、`input_barrier`を同時commit） |
...
| A19 | E25 | `rr_18_restart_no_replay` | offline | pass（startup reconcileでin-flight root/stepをfailed/interrupted、未消費proposalをexpired、queued/playing speechをcancelledへ遷移。新step 0） |
| A20 | E25 | `rr_14_subscribe_replay_race` | offline | pass（live購読をsnapshotより先に確立し、event root hintを含むroot-local replayを直列化。`(rootId,seq)`重複排除2件pass） |
| A21 | E19 | `rr_23_challenge_starts_new_root` | offline | pass（旧rootはcompletedのまま、新rootが旧回答へのfeedbackを保持） |
...
| A27 | E20 | `rr_26_approval_consumed_once` | offline | pass（未解決review→proposal→明示承認→一度だけの消費/step claim→bound premium executorを接続。曖昧承認、二重消費、能力失効は起動0） |
...
| A30 | E16/L01 | `rr_21_sol_tool_roundtrip` | offline/live | offline pass（実loopback gateway、role session、tool owner/ledger settle、host候補採用。認証済みSolはL01未許可） |
| A31 | E28 | `rr_30_feature_snapshot_immutable` | offline | pass（後日feedback追加後もfeatures/nullable quality不変） |
| A32 | E29 | `rr_31_crash_before_checkpoint` | offline | pass（fixture abortでdataset/job/cursor 0、dirty維持、再試行成功） |
| A33 | E30 | `rr_31_explicit_feedback_next_day` | offline | pass（新datasetのlabelRevisionへ反映、旧dataset不変） |
| A34 | E30 | `rr_32_silence_not_success` | offline | pass（沈黙/challenge/cancel/conflictはunknownかつ学習対象外） |
| A35 | E31 | `rr_33_group_split` | offline | pass（conversation groupの決定的train/eval分離） |
| A36 | E31 | `rr_33_partial_file_not_ready` | offline | pass（`rr_33_partial_file_not_ready_and_export_is_deterministic`） |
| A37 | E32 | `rr_34_missed_night` | offline | pass（missed window、日次一意、clock rollback、foreground page pause） |
| A38 | E33 | `rr_35_small_sample_rules` | offline | pass（eligible 20件未満はartifact 0） |
| A39 | E33 | `rr_35_shadow_no_second_call` | offline | pass（通常turnのProvider 1回、shadow観測1件、dispatch選択不変） |
| A40 | E34 | `rr_37_forget_invalidates_before_dispatch` | offline | pass |
| A41 | E22 | `rr_38_normal_turn_specialist_returns_to_parent` | offline | pass（通常provider turnのauthor→specialist typed request→host gateway/ledger→同じauthorへ結果返却。specialist最終採用0、重複operation再実行0） |
...
| --- | --- | --- | --- | --- | --- |
| P1 | host routing/DB 受付 p95<=50ms | E36 | `rr_39_host_receipt_p95_is_under_fifty_milliseconds` warm5+100 mock | offline | pass（in-memory SQLite、p95=403µs、閾値50,000µs） |
