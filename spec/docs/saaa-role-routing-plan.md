# SAAA Role Routing 実装計画 v3

更新日: 2026-09-21。状態: **部分実装あり・製品受入未完了。今回の更新は計画変更**。
...
最新の音声要件と直近の実装順は[音声会話統合計画（VC改訂）](saaa-role-routing-voice-integration.md)に定める。LFMは会話担当を継続し、Qwenには思考を依頼する。LFMから会話担当を委譲しない。最小JSONはsay/thinkの2項目、不安定なら平文LFM＋Qwen並走。最終回答の音声はQwenを優先する。
...
具体的な契約は実行契約C節と学習契約L節を正本とする。ただし音声受付・frontend出力・並走・発話についてはVC改訂が競合する旧記述に優先する。カードにない機能を推測で足さない。数値は初期製品設定の設計値であり、実測値ではない。実機のモデルID、接続能力、利用可能性はRR-00の証跡に記録する。未確認のモデルを利用可能として登録しない。
...
3. 実行中入力もLFMの会話受付へ送り、同意・相槌・進捗確認ではQwenを中断しない。同じ依頼への条件変更だけrootのbarrier・drain・revision更新へ進める。Providerの内部思考への直接追記はしない。
4. ツールの副作用は実行ownerが確定するまで保持。再考で二重実行しない。曖昧なremote outcomeはunknownとして停止する。
5. LFMは会話sessionの常時受付担当。1.5秒の無音で確定した入力へsay/thinkのみを返す。thinkは思考依頼であって会話担当の委譲ではない。平文並走への切替後もLFMを停止しない。モデルが権限や完了を確定することはない。
...
1. RR-00/VC00で基準を記録し、直近の音声修正はVC01〜VC08を依存順に実装する。既存の必要な採用・tool安全条件を再利用・補完する。残る全機能は完了ロードマップに従う。
