...
作成日: 2026-09-21。進捗更新: 2026-09-21。状態: **offline実装残件完了・製品受入未完了**。担当想定: Terra。

## 0. 現在の進捗と再開位置

残作業の実行手順は [Terra向け実装残件専用計画](saaa-world-model-remaining-terra-plan.md) を参照する。本書は全体の合格条件と進捗を保持する。

Terra残件計画T00〜T24のコードとoffline受入は完了した。共通Frame v2、五要素graph、Situation、Coding、委任、期限、自然文抽出、訂正・forget、全adapter、検証済み現在状態回答、Scope UIが接続済みである。6経路×7遷移は42/42合格し、追加処理p95は5.235001msだった。

実Providerを用いる30例、実UI、実音声、TTFA、プロジェクト全体の既存size/Clippy違反は製品受入として残る。詳細とraw条件は `spec/evidence/world-delivery/remaining-results.md` と `spec/evidence/world-delivery/route-matrix.md` を正本とする。

### 0.1 現在の判定

| 範囲 | 状態 | 残る作業 |
| --- | --- | --- |
| WD-01〜12の実装 | offline完了 | 稼働Provider・実UI・実音声による製品受入 |
| WD-13 offline | 完了 | 42セル、カードrunner、追加p95は合格 |
| WD-13 live | 未完了 | 6経路各5例、実UI/音声、TTFA悪化10%以内 |
| 全体品質 | 未完了 | World担当外を含む既存size/Clippy違反の解消 |

### 0.2 現在のlive blocker

- `192.168.0.130:8080` のLAN Providerは直近確認で接続不能。
- `127.0.0.1:44449` は応答するがローカルモデルがdisabledで、worker/modelは未起動。
- reasoning MCP v3対応serviceと実Codexモデルを同じ受入環境で起動する必要がある。

これらはlive受入だけのblockerであり、offline実装の未完了理由にはしない。

### 0.4 旧レビュー時点のカード表（履歴・現在の判定には使用しない）
