...
目標の中止や委任の撤回は、以後の実行判断へ反映する。実行途中の仕事についても、停止できる地点と既に生じた影響を追跡する。撤回後に到着した実行結果は新しい操作の許可に使わず、許可された最小限の実行証跡として元のTaskへ記録する。忘却による本文保存禁止は別に適用し、結果が失効したことと外部操作が実施されなかったことを混同しない。
...
共通の受入シナリオでは、project AのTask開始、Bへの会話切替、Aの完了通知、委任撤回、再起動、Aの続きの依頼を連続して試す。Scope漏洩、重複操作、撤回後の新規実行を0件とし、再説明回数、確認負荷、通知条件への適合、正しいTaskへの再開を測る。進行中Taskへの撤回と遅延結果の到着も別分岐で試し、再開要求だけで撤回済み委任を復活させない。個別機能の合格だけでこの循環の成立を代替しない。
