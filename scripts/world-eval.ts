...
// Invalidate old success before compiling, including interruption or a missing cargo executable.
await writeFile(
  resolve(directory, "m4a-report.json"),
...
for (const line of output.split("\n")) {
  ...
  try {
    ...
    if (
      ...
      report.cases.some((c: Case) => typeof c.case_id !== "string" || typeof c.pass !== "boolean")
    )
      throw new Error("invalid cases");
    cases.push(...report.cases);
  } catch {
    reportErrors.push("invalid suite report");
  }
}
const required = `
...
const report = {
  schema_version: 1,
  suite: "world-m4a",
  contract: "WD: AgentSession initial-only; G1 graph enabled",
  summary: {
    total: cases.length,
    passed,
