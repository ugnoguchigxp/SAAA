...
async function main(): Promise<void> {
  ...
  try {
    ...
    process.stdout.write(
      `${JSON.stringify({ format: REPORT_FORMAT, mode: outcome.mode, result: outcome.result })}\n`,
    );
    ...
    process.stdout.write(
      `${JSON.stringify({ format: REPORT_FORMAT, mode, result: known.result })}\n`,
    );
