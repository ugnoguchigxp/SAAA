...
export const count = (key: string, description: string, exact = 0): MetricSpec => ({
  key,
  unit: "count",
  description,
  exact,
});
export const atLeast = (key: string, description: string, min = 1): MetricSpec => ({
  key,
  unit: "count",
  description,
  min,
});
export const latency = (key: string, description: string, max = 5_000): MetricSpec => ({
  key,
  unit: "milliseconds",
  description,
  max,
...
export const INPUT_ACTIVITY_CASES: CaseSpec[] = [
  ...forBuilds(
    "monitoring-off",
    "monitoring offで10秒間OS input API hitが0件であることをsymbolic breakpointで確認する。",
    [count("osApiHitCount", "OS input API hit数")],
  ),
  ...forBuilds(
    "bounded-payload",
    "monitoring onでUI/contractにcategoryとhealthだけが現れることを確認する。",
    [
      count("rawDurationPayloadCount", "raw duration payload数"),
      count("lastInputTimestampPayloadCount", "last-input timestamp payload数"),
    ],
  ),
  ...forBuilds("keyboard-reset", "keyboard操作後30秒以内にactiveへ戻ることを確認する。", [
    latency("activeResetLatencyMs", "activeへのreset時間", 30_000),
  ]),
  ...forBuilds("mouse-reset", "mouse操作後30秒以内にactiveへ戻ることを確認する。", [
    latency("activeResetLatencyMs", "activeへのreset時間", 30_000),
  ]),
  ...forBuilds("trackpad-reset", "trackpad操作後30秒以内にactiveへ戻ることを確認する。", [
    latency("activeResetLatencyMs", "activeへのreset時間", 30_000),
  ]),
  ...forBuilds("recent-transition", "30秒超から5分以内にrecentへ遷移することを確認する。", [
    {
      key: "transitionSeconds",
      unit: "seconds",
      description: "recent遷移までの秒数",
      min: 31,
      max: 300,
    },
  ]),
  ...forBuilds("idle-transition", "5分超でidleへ遷移することを確認する。", [
    {
      key: "transitionSeconds",
      unit: "seconds",
      description: "idle遷移までの秒数",
      min: 301,
      max: 900,
    },
  ]),
  ...forBuilds(
    "lock-unlock",
    "screen lock/unlock後にbounded categoryまたはdegraded fallbackへ戻ることを確認する。",
    [
      count("recoveryFailureCount", "recovery失敗回数"),
      atLeast("boundedStateCount", "確認したbounded state数"),
    ],
  ),
  ...forBuilds(
    "sleep-resume",
    "sleep/resume後にpanicせずbounded categoryまたはdegraded fallbackへ戻ることを確認する。",
    [
      count("recoveryFailureCount", "recovery失敗回数"),
      atLeast("boundedStateCount", "確認したbounded state数"),
    ],
  ...
  ...forBuilds(
    "permission-prompts",
    "Accessibility/Input Monitoring permission promptが0件であることを確認する。",
    [count("permissionPromptCount", "permission prompt数")],
  ),
  {
    caseId: "sampling-soak",
    buildClass: "signed-packaged",
    instruction: "30分sampling soakのsample数とRSS増加を確認する。",
    metrics: [
      {
        key: "elapsedSeconds",
        unit: "seconds",
        description: "sampling継続時間",
        min: 1_800,
        automatic: "elapsed-seconds",
      },
      { key: "sampleCount", unit: "count", description: "sampling件数", min: 890 },
      {
        key: "rssGrowthMiB",
        unit: "mib",
        description: "RSS中央値増加量",
        max: 32,
  ...
  ...forBuilds(
    "privacy-scan",
    "SQLite、diagnostics、payload、frontend、console、reportのprivacy leakが0件であることを確認する。",
    [
      count("rawDurationLeakCount", "raw duration leak数"),
      count("lastInputTimestampLeakCount", "last-input timestamp leak数"),
    ],
  ),
];
...
export const AGENT_RUN_CASES: CaseSpec[] = [
  {
    ...
    buildClass: "signed-packaged",
    instruction:
      "normal read-only turnがstreamしterminal 1件、SQLite completedになることを確認する。",
    metrics: [
      { key: "terminalEventCount", unit: "count", description: "terminal event数", exact: 1 },
      { key: "completedRowCount", unit: "count", description: "completed row数", exact: 1 },
    ],
  },
  {
    caseId: "long-reasoning",
    buildClass: "signed-packaged",
    instruction: "meaningful progressを伴うlong reasoningがtimeoutせず完了することを確認する。",
    metrics: [
      atLeast("meaningfulProgressCount", "meaningful progress数", 2),
      count("falseProgressTimeoutCount", "誤progress timeout数"),
      { key: "terminalEventCount", unit: "count", description: "terminal event数", exact: 1 },
    ],
  },
  {
    caseId: "cancel-single",
    buildClass: "signed-packaged",
    instruction: "単一Cancelでinterrupt最大1、child/task残存0を確認する。",
    metrics: [
      { key: "interruptCount", unit: "count", description: "interrupt送信数", max: 1 },
      count("childProcessRemainingCount", "残存child process数"),
      count("asyncTaskRemainingCount", "残存async task数"),
    ],
  },
  {
    caseId: "cancel-repeated",
    buildClass: "signed-packaged",
    instruction: "Cancelを10回連打しinterrupt最大1、child/task残存0を確認する。",
    metrics: [
      { key: "cancelRequestCount", unit: "count", description: "Cancel操作回数", exact: 10 },
      { key: "interruptCount", unit: "count", description: "interrupt送信数", max: 1 },
      count("childProcessRemainingCount", "残存child process数"),
      count("asyncTaskRemainingCount", "残存async task数"),
    ],
  },
  {
    ...
    buildClass: "signed-packaged",
    instruction:
      "active run中の通常window closeでappとCodex childが5秒以内に終了することを確認する。",
    metrics: [
      latency("appProcessExitMs", "app process終了時間"),
      latency("childProcessExitMs", "Codex child終了時間"),
      count("runningRowRemainingCount", "残存running row数"),
    ],
  },
  {
    caseId: "forced-restart",
    buildClass: "signed-packaged",
    instruction:
      "強制終了後のrestartでrunning rowがinterrupted/app-restartedになることを確認する。",
    metrics: [
      count("runningRowRemainingCount", "残存running row数"),
      atLeast("interruptedRowCount", "interrupted row数"),
      atLeast("appRestartedReasonCount", "app-restarted reason数"),
    ],
  },
  {
    caseId: "read-only-violation",
    buildClass: "signed-packaged",
    instruction:
      "workspace変更要求が拒否され、approval/Network/Web Search/write MCPが0件であることを確認する。",
    metrics: [
      count("workspaceChangeCount", "workspace変更数"),
      count("approvalUiCount", "approval UI表示数"),
      count("networkRequestCount", "Network request数"),
      count("webSearchCount", "Web Search数"),
      count("writeMcpCount", "write-capable MCP数"),
    ],
  },
].map((spec) => ({
  ...spec,
  metrics: [
    ...spec.metrics,
    {
      ...count("workspaceDigestMismatchCount", "fixture workspace digest不一致数"),
      automatic: "workspace-integrity",
    },
    count("rawProviderLeakCount", "raw provider data leak数"),
  ],
}));
  ...
  .superRefine((value, context) => {
    if (Date.parse(value.completedAt) < Date.parse(value.startedAt))
      context.addIssue({
        code: "custom",
        path: ["completedAt"],
        message: "completedAt precedes startedAt",
      });
    if ((value.result === "pass") !== (value.reasonCode === null))
      context.addIssue({
        code: "custom",
        path: ["reasonCode"],
        message: "pass requires null reason and non-pass requires a reason",
      });
    if (value.result === "pass" && value.observations.length === 0)
      context.addIssue({
        code: "custom",
        path: ["observations"],
        message: "pass requires bounded observations",
      });
  });
...
export const suiteReportSchema = z
  ...
  .superRefine((report, context) => {
    ...
    if (new Set(keys).size !== keys.length)
      context.addIssue({ code: "custom", path: ["cases"], message: "duplicate case result" });
    const derived = report.cases.some((item) => item.result === "fail")
      ? "fail"
      : report.cases.some((item) => item.result === "blocked")
        ? "blocked"
        : "pass";
    if (report.result !== derived)
      context.addIssue({
        code: "custom",
        path: ["result"],
        message: "suite result disagrees with cases",
      });
  });
...
export const aggregateReportSchema = z
  ...
  .superRefine((report, context) => {
    const accepted =
      report.passedCaseCount === report.expectedCaseCount &&
      report.failedCaseCount === 0 &&
      report.blockedCaseCount === 0 &&
      report.missingCaseCount === 0 &&
      report.forbiddenDataFindingCount === 0;
    if ((report.result === "accepted") !== accepted)
      context.addIssue({
        code: "custom",
        path: ["result"],
        message: "aggregate result disagrees with counts",
      });
  });
