...
function verify(directory: string): void {
    ...
    ? [
        {
          ...
          elapsedMs: 0,
          passed: readBenchmark(join(directory, "sqlite-read-path.json")).passed,
        },
      ]
    : [
        run(
          "sqlite-read-path",
          "cargo",
          ["bench", "--manifest-path", "src-tauri/Cargo.toml", "--bench", "sqlite_read_path"],
          environment,
        ),
      ];
  const commands = [
    ...
    run("frontend-hot-path", "bun", [
      "test",
      "tests/streaming-text-buffer.test.ts",
      "tests/final-markdown.test.ts",
      "tests/streaming-performance.test.ts",
      "tests/meeting-audio-worker.test.ts",
      "tests/interrupted-streaming-ui.test.ts",
    ]),
  ];
  ...
  writeFileSync(
    join(directory, "automated-verification.json"),
