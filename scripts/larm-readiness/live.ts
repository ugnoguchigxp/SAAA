  ...
  while (performance.now() < checkpointDeadline) {
    if (application.child.exitCode !== null || applicationDetectedForbiddenData(application)) {
      throw new RunnerError(
        2,
        applicationDetectedForbiddenData(application)
          ? "redaction-failed"
...
export function withCanaryDatabase<T>(
...
export function observeDatabase(
  ...
  return withCanaryDatabase(environment.dataDirectory, (database) =>
    databaseObservation(database, snapshot),
  );
}

export function observeSettings(environment: ValidatedEnvironment): SettingsState {
  return withCanaryDatabase(environment.dataDirectory, (database) =>
    settingsState(database, environment.manifest),
  );
}
...
export function validateFunctionalObservation(
  ...
  const larm = observation.sessions.filter(
    (session) => session.provider_id === larmId && session.provider_kind === "larm",
  );
  const direct = observation.sessions.filter(
    (session) =>
      session.provider_id === "local-openai-compatible" &&
      session.provider_kind === "openai-compatible",
  );
  const failureKinds = larm
    .filter((session) => session.status === "failed")
    .map((session) => session.failure_kind)
    .sort();
  const failureByKind = (kind: string) =>
    larm.find((session) => session.status === "failed" && session.failure_kind === kind);
  const statusCounts = (status: string) =>
    observation.runs.filter((run) => run.status === status).length;
  const allocationIds = larm.flatMap((session) =>
    session.allocation_id === null ? [] : [session.allocation_id],
  );
  const selectedRuntimes = larm.flatMap((session) =>
    session.selected_runtime_id === null ? [] : [session.selected_runtime_id],
  );
  const runIds = new Set(observation.runs.map((run) => run.id));
  const releaseInvalid = larm.some(
    (session) =>
      session.allocation_id !== null &&
      !["released", "deferred-to-ttl"].includes(session.release_status),
  );
  ...
  if (
    ...
    observation.sessions.some(
      (session) => session.runtime_run_id === null || !runIds.has(session.runtime_run_id),
    ) ||
    observation.runs.some(
      (run) => !observation.sessions.some((session) => session.runtime_run_id === run.id),
    ) ||
      ...
      if (run.status === "completed") {
        return !(
          ...
          (sessions.length === 2 &&
            sessions.some(
              (session) =>
                session.provider_id === larmId &&
                session.status === "failed" &&
                ["capacity", "timeout"].includes(session.failure_kind ?? ""),
            ) &&
            sessions.some(
              (session) =>
                session.provider_id === "local-openai-compatible" && session.status === "completed",
            ))
        );
      }
...
export async function observeFunctional(
  ...
  try {
    ...
    await waitForCheckpoint(
      application,
      "enabling-larm-canary",
      () => observeSettings(environment).larmPrimary,
      deadlineAt,
    );
    ...
    await waitForCheckpoint(
      application,
      "waiting-for-ui-workload",
      () => {
        const observation = observeDatabase(environment, snapshot);
        return (
          countSessions(
            observation,
            (session) => session.provider_id === larmId && session.status === "completed",
          ) >= 1 &&
          countSessions(
            observation,
            (session) => session.provider_id === larmId && session.status === "cancelled",
          ) >= 1
        );
      },
      deadlineAt,
    ...
    await waitForCheckpoint(
      application,
      "checkpoint-timeout-fixture-ready",
      () => {
        const observation = observeDatabase(environment, snapshot);
        return (
          countSessions(
            observation,
            (session) => session.provider_id === larmId && session.failure_kind === "timeout",
          ) >= 1 &&
          countSessions(
            observation,
            (session) =>
    ...
    await waitForCheckpoint(
      application,
      "checkpoint-tunnel-interruption-ready",
      () => {
        const observation = observeDatabase(environment, snapshot);
        return (
          countSessions(
            observation,
            (session) =>
              session.provider_id === larmId && session.failure_kind === "partial-output",
          ) >= 1 &&
          countSessions(
            observation,
            (session) => session.provider_id === larmId && session.status === "completed",
    ...
    await waitForCheckpoint(
      application,
      "checkpoint-capacity-fixture-ready",
      () => {
        const observation = observeDatabase(environment, snapshot);
        return (
          countSessions(
            observation,
            (session) => session.provider_id === larmId && session.failure_kind === "capacity",
          ) >= 1 &&
          countSessions(
            observation,
            (session) =>
      ...
      try {
        ...
        if (
          ...
          (cleanupError instanceof RunnerError &&
            ...
            (!(primaryError instanceof RunnerError) ||
              primaryError.errorCode !== "redaction-failed"))
        ) {
          throw cleanupError;
        }
      }
  ...
  const deadline = new Promise<never>((_, reject) => {
    timer = setTimeout(() => {
      void terminateOwnedChild(child).finally(() =>
        reject(new RunnerError(2, "sampling-gap", "failed")),
      );
    }, 2_000);
  });
  try {
...
export function validateSoakObservation(
  observation: DatabaseObservation,
  larmId: string,
  ...
  const sessions = observation.sessions.filter(
    (session) => session.provider_id === larmId && session.provider_kind === "larm",
  );
  const allocationIds = sessions.flatMap((session) =>
    session.allocation_id === null ? [] : [session.allocation_id],
  );
  const runIds = new Set(observation.runs.map((run) => run.id));
  const sessionRunIds = sessions.flatMap((session) =>
    session.runtime_run_id === null ? [] : [session.runtime_run_id],
  );
  if (
    ...
    sessions.some(
      (session) =>
        ...
        (session.status === "completed"
          ? session.failure_kind !== null
          : session.failure_kind !== "cancelled"),
    ) ||
    sessions.some(
      (session) =>
        session.selected_runtime_id === null ||
    ...
    sessions.some(
      (session) => session.route_id !== "llm-default" || session.selection_reason !== "primary",
    ) ||
    sessions.some(
      (session) =>
        session.allocation_id === null ||
...
export async function observeSoak(
  ...
  try {
    while (performance.now() - started < durationMs) {
      ...
      nextSampleAt += 5_000;
      if (nextSampleAt < performance.now() - 15_000)
        throw new RunnerError(2, "sampling-gap", "failed");
    }
    ...
    const memory = firstProcessRss.filter(
      (sample) => sample.elapsedMs >= memoryStart && sample.elapsedMs <= memoryEnd,
    );
    if (memory.length === 0) throw new RunnerError(2, "sampling-gap", "failed");
    const rssRangeMiB = Math.ceil(
      (Math.max(...memory.map((sample) => sample.rssKiB)) -
        Math.min(...memory.map((sample) => sample.rssKiB))) /
        1_024,
    );
    let previousMedianMiB = 0;
    let lastMedianMiB = 0;
    if (mode === "soak-2h") {
      previousMedianMiB = Math.ceil(
        median(
          firstProcessRss
            .filter((sample) => sample.elapsedMs >= 10 * 60_000 && sample.elapsedMs < 40 * 60_000)
            .map((sample) => sample.rssKiB),
        ) / 1_024,
      );
      lastMedianMiB = Math.ceil(
        median(
          firstProcessRss
            .filter((sample) => sample.elapsedMs >= 40 * 60_000 && sample.elapsedMs <= 70 * 60_000)
            .map((sample) => sample.rssKiB),
        ) / 1_024,
      );
    }
