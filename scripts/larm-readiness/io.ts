...
export function assertSeparated(paths: string[]): void {
  for (let left = 0; left < paths.length; left += 1) {
    for (let right = left + 1; right < paths.length; right += 1) {
      if (
        isAncestorOrSame(paths[left]!, paths[right]!) ||
        isAncestorOrSame(paths[right]!, paths[left]!)
      ) {
        throw new RunnerError(3, "environment-invalid", "blocked");
      }
    }
        ...
        : command === "canary"
          ...
          : command === "soak" && duration === "30m"
            ...
            : command === "soak" && duration === "2h"
              ? [
                  REPORT_FILENAMES.preflight,
                  REPORT_FILENAMES.functional,
                  REPORT_FILENAMES["soak-30m"],
                ]
              : [
                  REPORT_FILENAMES.preflight,
                  REPORT_FILENAMES.functional,
                  REPORT_FILENAMES["soak-30m"],
                  REPORT_FILENAMES["soak-2h"],
                ];
    if (
...
export function sameFileIdentity(left: StableFileIdentity, right: StableFileIdentity): boolean {
  return (Object.keys(left) as Array<keyof StableFileIdentity>).every(
...
export function loadManifestUnchecked(
  filename: string,
  phase: "preflight" | "functional" | "later",
  repositoryRoot = ROOT,
): { manifest: CanaryManifest; sha256: string; canonicalFile: string } {
  ...
  const normalData = existsSync(normalDataPath)
    ? realpathSync(normalDataPath)
    : resolve(normalDataPath);
  if (
    canonicalData === home ||
  ...
  if (
    phase === "preflight" || phase === "functional"
      ? dataEntries.length !== 0
      : !dataEntries.includes("saaa.sqlite3")
  ) {
    throw new RunnerError(3, "environment-invalid", "blocked");
  }
  return {
    manifest: { ...manifest, dataDirectory: canonicalData },
...
export function assertManifestSeparation(
  manifestFile: string,
...
export function validateLiveEnvironmentUnchecked(
  ...
  if (
    !deployedCommit.success ||
    !deploymentRevision.success ||
    manifestFilename === undefined ||
    environmentReportDirectory === undefined ||
    environmentDataDirectory === undefined
  ) {
    throw new RunnerError(3, "environment-invalid", "blocked");
  }
  const canonicalReport = realpathSync(reportDirectory);
  if (realpathSync(environmentReportDirectory) !== canonicalReport)
    throw new RunnerError(3, "environment-invalid", "blocked");
  ...
  const rollbackCredential =
    rawRollbackCredential === undefined
      ? undefined
...
export function gitCommitSync(): string {
  ...
  if (result.exitCode !== 0 || !commit.safeParse(value).success)
...
export const RUST_BASE_ENVIRONMENT = [
  "PATH",
  "HOME",
  "TMPDIR",
  "TEMP",
...
export function rustChildEnvironment(
  environment: Record<string, string | undefined>,
  internal: {
    resultFile: string;
  ...
  const result: Record<string, string> = {};
  for (const key of RUST_BASE_ENVIRONMENT)
    if (environment[key] !== undefined) result[key] = environment[key]!;
  for (const key of [
    "SAAA_LARM_CANARY",
    "SAAA_LARM_ENABLED",
...
export function appChildEnvironment(
  ...
  for (const key of [
    "SAAA_LARM_CANARY",
    "LARM_API_TOKEN",
    "SAAA_PROVIDER_LOCAL_OPENAI_COMPATIBLE_API_KEY",
  ] as const) {
    if (environment[key] !== undefined) result[key] = environment[key]!;
  }
...
export class ForbiddenDataScanner {
  ...
  scan(chunk: Uint8Array): boolean {
    ...
    this.failed ||=
      this.exact.some((needle) => bytes.includes(needle)) ||
      /authorization\s*:/i.test(text) ||
      /bearer\s+[\x21-\x7e]+/i.test(text) ||
      /(?:allocation|operation|request|conversation|runtime[_ -]?run)[_ -]?id["']?\s*[=:]/i.test(
        text,
      );
