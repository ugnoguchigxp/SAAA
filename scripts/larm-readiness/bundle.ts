...
export function hashBundleFile(file: BundleFile): Buffer {
  ...
  try {
    ...
    if (
      ...
      before.nlink !== 1 ||
      !sameFileIdentity(before, file.identity)
    ) {
      throw new RunnerError(2, "artifact-mismatch", "failed");
    }
...
export async function canonicalBundleDigest(bundle = RELEASE_BUNDLE): Promise<string> {
  try {
    ...
    if (!rootInfo.isDirectory() || rootInfo.isSymbolicLink())
      throw new RunnerError(2, "artifact-mismatch", "failed");
    const executable =
      bundle === RELEASE_BUNDLE ? RELEASE_EXECUTABLE : join(bundle, "Contents/MacOS/saaa");
    ...
    const files = collectBundleFiles(bundle).sort((left, right) =>
    ...
    const finalFiles = collectBundleFiles(bundle).sort((left, right) =>
      Buffer.compare(left.relativePath, right.relativePath),
    );
    if (!sameBundleInventory(files, finalFiles))
      throw new RunnerError(2, "artifact-mismatch", "failed");
...
export function atomicWriteReport(filename: string, reportInput: ReadinessReport): void {
  ...
  if (bytes.length > MAX_REPORT_BYTES || existsSync(filename))
    throw new RunnerError(2, "report-schema-invalid", "failed");
  assertNoSymlinkComponents(dirname(filename));
  const temporary = join(
    dirname(filename),
    `.${basename(filename)}.${randomBytes(16).toString("hex")}.tmp`,
  );
  try {
...
export function readFinalReportWithBytes(filename: string): {
...
export function aggregateReports(inputs: ReadinessReport[]): ReadinessReport {
  ...
  if (modes.some((mode, index) => reports[index]!.mode !== mode))
    throw new RunnerError(2, "report-schema-invalid", "failed");
  ...
  const allocationBaselineValid = reports.every(
    (report) =>
      report.resourceSummary.baselineActiveAllocations ===
        first.resourceSummary.baselineActiveAllocations &&
      report.resourceSummary.finalActiveAllocations ===
        first.resourceSummary.baselineActiveAllocations,
  );
  const sum = <T extends readonly string[]>(
    keys: T,
    getter: (report: ReadinessReport) => Record<T[number], number>,
  ) =>
    Object.fromEntries(
      keys.map((key) => {
        const value = reports.reduce((total, report) => total + getter(report)[key], 0);
        if (!Number.isSafeInteger(value) || value > 10_000)
          throw new RunnerError(2, "report-schema-invalid", "failed");
        return [key, value];
      }),
    ) as Record<T[number], number>;
  const sourceResult = reports.reduce(
    (strongest, report) =>
      resultStrength(report.result) > resultStrength(strongest) ? report.result : strongest,
    "passed" as Result,
  );
  const result: Result = allocationBaselineValid ? sourceResult : "failed";
  const aggregate: ReadinessReport = {
    ...first,
    startedAt: reports.reduce(
      (earliest, report) =>
        utcTimestampSortKey(report.startedAt) < utcTimestampSortKey(earliest)
          ? report.startedAt
          : earliest,
      first.startedAt,
    ),
    finishedAt: reports.reduce(
      (latest, report) =>
        utcTimestampSortKey(report.finishedAt) > utcTimestampSortKey(latest)
          ? report.finishedAt
          : latest,
      first.finishedAt,
    ),
    ...
    timingSummary: Object.fromEntries(
      TIMING_KEYS.map((key) => [
        key,
        Math.max(...reports.map((report) => report.timingSummary[key])),
      ]),
    ) as ReadinessReport["timingSummary"],
    resourceSummary: {
      baselineActiveAllocations: first.resourceSummary.baselineActiveAllocations,
      maxActiveAllocations: Math.max(
        ...reports.map((report) => report.resourceSummary.maxActiveAllocations),
      ),
    ...
    redactionCheck: reports.some((report) => report.redactionCheck === "failed")
      ? "failed"
      : "passed",
    result,
  };
...
export function assertPredecessor(
  ...
  identity: Omit<
    ...
    | "dataDirectory"
    | "reportDirectory"
  > & { artifactSha256: string; saaaCommit: string },
): ReadinessReport | undefined {
  const predecessors: LiveMode[] =
    mode === "preflight"
      ? []
      : mode === "functional"
        ? ["preflight"]
...
export async function runRustLiveSuite(
  mode: LiveMode,
  ...
  if (child.exitCode !== 0 || !existsSync(resultFilename))
    throw new RunnerError(2, "internal", "failed");
...
export async function releaseArtifact(
  ...
  deadlineAt: number,
): Promise<{ artifactSha256: string; saaaCommit: string }> {
  ...
  if (command === "preflight") {
    ...
    if (assertCleanRepository() !== saaaCommit)
      throw new RunnerError(2, "artifact-mismatch", "failed");
  }
