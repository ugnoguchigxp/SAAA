...
export const databaseIdentifier = z
  .string()
  .min(1)
  .max(160)
  .regex(/^[A-Za-z0-9_-]+$/);
export const databaseProviderId = z
  .string()
  .min(1)
  .max(80)
  .regex(/^[A-Za-z0-9_-]+$/);
export const runtimeStatus = z.enum(["running", "completed", "failed", "cancelled", "interrupted"]);
export const runtimeRowSchema = z
  .object({
  ...
  .strict();
export const providerSessionRowSchema = z
  .object({
    id: databaseIdentifier,
...
export function openCanaryDatabase(dataDirectory: string): Database {
  ...
  try {
    ...
    if (!sameFileObject(initialIdentity, stableFileIdentity(openedInfo)))
      throw new Error("database changed while opening");
    ...
    for (const [table, columns] of Object.entries(required)) {
      const entry = database.query("SELECT type FROM sqlite_master WHERE name=?1").get(table) as {
        type: string;
      } | null;
...
export function settingsState(database: Database, manifest: CanaryManifest): SettingsState {
  ...
  try {
    ...
    const providersDocument = documents.find(
      (document) => document.namespace === "providers.model",
    );
    const routingDocument = documents.find((document) => document.namespace === "routing.tasks");
    if (providersDocument === undefined || routingDocument === undefined)
    ...
    const direct = providers.find(
      (provider) =>
        provider.kind === "openai-compatible" && provider.id === "local-openai-compatible",
    );
    if (
      larm === undefined ||
      direct === undefined ||
      ...
      larm.allocationStartupTimeoutSeconds !==
        manifest.larmProvider.allocationStartupTimeoutSeconds ||
    ...
    const state = {
      larmId: larm.id,
      directPrimary:
        route.primaryProviderId === direct.id && route.fallbackProviderIds.length === 0,
      larmPrimary:
        route.primaryProviderId === larm.id &&
...
export function databaseSnapshot(database: Database): DatabaseSnapshot {
  ...
  try {
    database.exec("BEGIN");
    transactionOpen = true;
    const runtimeRows = database.query("SELECT id FROM runtime_runs LIMIT 10001").all() as Array<{
      id: string;
    }>;
    const sessionRows = database
      .query("SELECT id FROM provider_sessions LIMIT 10001")
      .all() as Array<{ id: string }>;
    if (runtimeRows.length > 10_000 || sessionRows.length > 10_000)
      throw new Error("snapshot too large");
    const parsedRuntimeRows = z
      .array(z.object({ id: databaseIdentifier }).strict())
      .max(10_000)
      .parse(runtimeRows);
    const parsedSessionRows = z
      .array(z.object({ id: databaseIdentifier }).strict())
      .max(10_000)
      .parse(sessionRows);
    const snapshot = {
...
export function databaseObservation(
  database: Database,
  ...
  try {
    database.exec("BEGIN");
    transactionOpen = true;
    const rawRuns = database
      .query("SELECT id,conversation_id,provider_id,status FROM runtime_runs LIMIT 10001")
      .all();
    const rawSessions = database
      .query(
      ...
      .all();
    if (rawRuns.length > 10_000 || rawSessions.length > 10_000)
