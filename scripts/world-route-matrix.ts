...
export function validateMatrix(cases: MatrixCase[]): string[] {
  ...
  for (const entry of cases) {
    ...
    if (entry.case_id !== `${entry.route}:${entry.transition}`) errors.push(`invalid route matrix identity ${identity}`);
    ...
    if (isContract) {
      if (identity !== "reasoning-mcp:tool-continuation"
    ...
    if (denied) {
      if (entry.omission_reason !== "scope-changed"
        || entry.request_count !== 0
        || entry.frame_digest !== null
        || entry.wire_digest !== null
    ...
    const sourceKinds = Array.isArray(entry.source_kinds)
      ...
      : "";
    const minimumRequests = ["tool-continuation", "fallback", "session-resume"].includes(entry.transition) ? 2 : 1;
    if (entry.omission_reason !== null
