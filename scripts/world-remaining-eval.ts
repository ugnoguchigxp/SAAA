...
export async function runRemaining() {
  ...
  const matrixParseErrors: string[] = [];
  ...
  for (const manifest of ["crates/personal-state-core/Cargo.toml", "src-tauri/Cargo.toml"]) {
    ...
    const parsedMatrix = parseMatrixOutput(output);
    matrix.push(...parsedMatrix.cases);
    matrixParseErrors.push(...parsedMatrix.errors.map((error) => `${manifest}: ${error}`));
  }
