...
async function plistValue(root: string, infoPlist: string, key: string) {
  const check = Bun.spawn(["/usr/bin/plutil", "-extract", key, "raw", "-o", "-", infoPlist], {
    cwd: root,
    stdout: "pipe",
    stderr: "pipe",
  });
...
export async function verifyMacBundle(root: string): Promise<void> {
  ...
  if (microphonePurpose.exitCode !== 0 || microphonePurpose.value.length === 0) {
    throw new Error(
      "Desktop smoke failed: packaged Info.plist has no microphone usage description.",
    );
  }
  if (
  ...
  if (
    (await entitlements.exited) !== 0 ||
    !entitlementXml.includes("com.apple.security.device.audio-input")
  ) {
