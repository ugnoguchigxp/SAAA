...
test("wr_t22_route_matrix_requires_each_route_transition_once", () => {
  ...
  const lines = routes.flatMap((route) => transitions.map((transition) => {
    ...
    return `WORLD_MATRIX_CASE=${JSON.stringify({
      case_id:`${route}:${transition}`, route, transition, pass:true,
      ...
      expected:contract ? "n/a-no-host-tool-capability" : denied ? "denied-before-model-wire" : "current-frame-and-matching-receipt",
      actual:contract ? "n/a-no-host-tool-capability" : denied ? "denied-before-model-wire" : "current-frame-and-matching-receipt",
      request_count:contract || denied ? 0 : ["tool-continuation","fallback","session-resume"].includes(transition) ? 2 : 1,
  ...
  expect(parseMatrixOutput(`${lines[0]}\nWORLD_MATRIX_CASE={broken`).errors)
    .toEqual(["invalid route matrix JSON at output line 2"]);
  expect(validateMatrix(matrix.map((entry, index) => index === 0 ? {...entry, pass:"true"} : entry) as never))
