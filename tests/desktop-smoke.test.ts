...
import { runDesktopSmoke, sanitizeSmokeLog, type SmokeOptions } from "../scripts/desktop-smoke-process";
...
function options(script: string): SmokeOptions {
  ...
  return { root: directory, reportDir: join(directory, "report"),
    build: [process.execPath, "-e", "console.log('build fixture')"],
    executable: [process.execPath, "-e", script], readyTimeoutMs: 500 };
}
...
function report(opts: SmokeOptions) { return JSON.parse(readFileSync(join(opts.reportDir, "summary.json"), "utf8")); }
...
describe("desktop smoke process lifecycle", () => {
  test("reports ready, captures logs, and cleans up the live application and data", async () => {
    const opts = options(identify + `fs.writeFileSync(JSON.parse(fs.readFileSync('child.json')).marker, 'ready'); console.log('application fixture'); setInterval(()=>{},1000);`);
    await runDesktopSmoke(opts);
    expect(report(opts).status).toBe("ready");
    expect(readFileSync(join(opts.reportDir, "application.stdout.log"), "utf8")).toContain("application fixture");
    assertClean(opts);
  });
  ...
  test("reports early application exit and its stderr", async () => {
    ...
    expect(readFileSync(join(opts.reportDir, "application.stderr.log"), "utf8")).toContain("fixture failure");
    assertClean(opts);
  });
  test("preserves build failure diagnostics without launching the app", async () => {
    ...
    expect(readFileSync(join(opts.reportDir, "build.stderr.log"), "utf8")).toContain("broken build");
  ...
  test("reports spawn and bundle failures", async () => {
    ...
    bundle.verifyBundle = async () => { throw new Error("invalid bundle"); };
  ...
  test("terminates a timed-out build and its child process group", async () => {
    const opts = options(identify);
    opts.buildTimeoutMs = 300;
    opts.build = [process.execPath, "-e", `const {spawn}=require('node:child_process');const fs=require('node:fs');const c=spawn(process.execPath,['-e','setInterval(()=>{},1000)'],{stdio:'inherit'});fs.writeFileSync('descendant',String(c.pid));setInterval(()=>{},1000);`];
  ...
  test("redacts configured secrets, workspace paths and URLs in diagnostics", () => {
    ...
    try {
      const redacted = sanitizeSmokeLog("smoke-secret-fixture /fixture/work/file https://host/path?token=x", "/fixture/work");
