...
test('failed lazy UI import preserves the surrounding conversation and summary', async () => {
  ...
  names.forEach((name,i) => Object.defineProperty(globalThis,name,{configurable:true,writable:true,value:values[i]}));
  const root = createRoot(dom.window.document.getElementById('root')!, {onCaughtError: () => {}});
  try {
    const Broken = lazy(async () => { throw Error('chunk unavailable'); });
    await act(async () => { root.render(<><p>既存の会話</p><UiBoundary fallback={<p>保存済みの要約</p>}><Suspense fallback="loading"><Broken /></Suspense></UiBoundary></>); });
    ...
    names.forEach((name,i) => { const descriptor = before[i]; if (descriptor) Object.defineProperty(globalThis,name,descriptor); else Reflect.deleteProperty(globalThis,name); });
  }
});
