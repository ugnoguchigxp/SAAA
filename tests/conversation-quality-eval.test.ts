...
} from "../scripts/conversation-quality/protocol";
...
import {
  ...
  validateScenarios,
  type Scores,
} from "../scripts/conversation-quality/schema";
...
describe("conversation quality evaluation contract", () => {
  ...
  test("rejects scores outside the rubric and totals valid scores", () => {
    ...
    expect(() =>
      parseEvaluation({ scores: { ...valid.scores, tools: 11 }, violations: [] }),
    ).toThrow("invalid tools score");
    expect(() =>
      parseEvaluation({ scores: valid.scores, violations: ["raw response content"] }),
    ).toThrow("fixed violation codes");
  });

  test("requires tool fixtures for current-information cases", () => {
    const broken = QUALITY_SCENARIOS.map((scenario) =>
      scenario.id === "current-01" ? { ...scenario, toolResult: undefined } : scenario,
    );
  ...
  test("rejects incomplete, duplicate, and internally inconsistent result matrices", () => {
    ...
    expect(() => summarizeQualityGate(complete.slice(1), scenarioIds)).toThrow(
      "incomplete result matrix",
    );
    expect(() =>
      summarizeQualityGate([...complete.slice(0, -1), complete[0]!], scenarioIds),
    ).toThrow("duplicate quality result");
    expect(() =>
      summarizeQualityGate([{ ...complete[0]!, score: 99 }, ...complete.slice(1)], scenarioIds),
    ).toThrow("does not match its rubric");
  });
  ...
  test("validates endpoint and web-search tool protocol", () => {
    ...
    expect(() => normalizeEndpointBaseUrl("https://user:secret@example.test/v1")).toThrow(
      ...
      parseWebSearchToolCall([
        {
          id: "call_1",
          type: "function",
          function: { name: "web_search", arguments: '{"query":"current weather"}' },
        },
      ]).id,
    ).toBe("call_1");
    expect(() =>
    ...
    ).toThrow("must invoke web_search");
    expect(() =>
      parseWebSearchToolCall([
        {
          id: "call_1",
          type: "function",
          function: { name: "web_search", arguments: '{"query":"weather","extra":true}' },
        },
      ]),
    ).toThrow("contain only query");
    expect(() => normalizeEndpointBaseUrl("not a url")).toThrow("valid URL");
    expect(() => parseWebSearchToolCall([])).toThrow("exactly one");
    expect(() =>
      parseWebSearchToolCall([
        { id: "", type: "function", function: { name: "web_search", arguments: '{"query":"q"}' } },
      ]),
