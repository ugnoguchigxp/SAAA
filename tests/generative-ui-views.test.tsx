...
function instance(mode: "live" | "snapshot" = "snapshot"): UiInstance {
  return {
    ...
    state: {},
    snapshots: {
      "runtime.summary": { capturedAt: "0", rows: [{ running: 1, completed: 2, failed: 0, total: 3 }] },
      "runtime.history": { capturedAt: "1", rows: [{ time: 0, count: 1 }, { time: 1_000, count: 4 }] },
      "runtime.runs": { capturedAt: "2", rows: [{ id: "run-1", status: "running", provider: "local", startedAt: 0 }] },
      "larm.status": { capturedAt: "3", rows: [{ provider: "larm", runtime: "qwen", status: "ready", updatedAt: 0 }] },
    },
    stateVersion: 0,
    name: null,
...
describe("generative UI views", () => {
  ...
  test("renders semantic nodes and interactive snapshot views", async () => {
    ...
    const tree = createElement(UiContext.Provider, { value },
      createElement(SemanticRenderer, { node: node("Grid", [], [
    ...
    await act(async () => sort?.dispatchEvent(new dom!.window.MouseEvent("click", { bubbles: true })));
    await act(async () => sort?.dispatchEvent(new dom!.window.MouseEvent("click", { bubbles: true })));
  ...
  test("uses live context hooks and snapshot-only actions", async () => {
    ...
    root = createRoot(document.getElementById("root")!);
    await act(async () => root!.render(createElement(UiContext.Provider, { value: live },
      createElement(MetricView, { source: "runtime.summary", field: "running", label: "Running" }),
    )));
    await act(async () => { await Promise.resolve(); });
    const snapshot = { instance: instance("snapshot"), conversationId: "c1", active: false, enabled: false };
    await act(async () => root!.render(createElement(UiContext.Provider, { value: snapshot },
      createElement(ActionsView, { action: "refresh" }),
    )));
    await act(async () => root!.render(createElement(UiBoundary, {
