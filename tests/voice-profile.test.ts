...
describe("target-speaker voice profile", () => {
  ...
  test("bundles only checksum-pinned local verification artifacts", async () => {
    const installer = await readFile(
      new URL("../scripts/install-speaker-runtime.sh", import.meta.url),
      "utf8",
    );
    ...
    const backend = [
      ...
      await readFile(
        new URL("../src-tauri/src/voice/profile/streaming_verifier.rs", import.meta.url),
        "utf8",
      ),
    ].join("\n");
    ...
    expect(backend).not.toMatch(
      /security_framework|load_master_key|encrypt_payload|decrypt_payload/,
    );
    for (const marker of [
      'format!("{sample_id}.wav")',
