...
test("capture restarts share one connection and stopping during startup rejects the stale capture", async () => {
  ...
  const owners = new LarmVoiceOwners(async (name) => {
    calls.push(name);
  ...
  expect(calls).toEqual(["begin_larm_voice_session", "end_larm_voice_session", "end_larm_voice_session"]);
});

test("late failure releases the original owner without touching its replacement", async () => {
  const calls: [string, Record<string, unknown>][] = [];
  let index = 0;
  const owners = new LarmVoiceOwners(async (name, args) => { calls.push([name, args]); }, () => `owner-${++index}`);
...
test("out-of-order start completion is released even if an earlier end saw no backend owner", async () => {
  ...
  const gate = new Promise<void>((resolve) => { finishStart = resolve; });
  const owners = new LarmVoiceOwners(async (name, args) => {
    if (name === "begin_larm_voice_session") { await gate; backendOwner = args.ownerId as string; }
...
test("a stale capture for another conversation never gets the new owner's lease", async () => {
  const calls: string[] = [];
  const owners = new LarmVoiceOwners(async (name) => { calls.push(name); }, () => "owner");
...
test("replacing an owner directly still releases the previous connection first", async () => {
  const calls: string[] = []; let id = 0;
  const owners = new LarmVoiceOwners(async (name, args) => { calls.push(`${name}:${args.ownerId}`); }, () => `owner-${++id}`);
  owners.own("old"); await owners.prepare("old");
  owners.own("new"); await owners.prepare("new");
  expect(calls).toEqual(["begin_larm_voice_session:owner-1", "end_larm_voice_session:owner-1", "begin_larm_voice_session:owner-2"]);
});
