...
describe("ServiceConnectionsSection asynchronous state", () => {
  test("does not apply a resolution for an address that has since changed", async () => {
    ...
    const pending = new Promise<HarnessResolution>((resolve) => {
      resolveRequest = resolve;
    });
    let providers = structuredClone(defaultSettingsDraft.providers);
    const render = () =>
      React.createElement(ServiceConnectionsSection, {
        providers,
        routing: defaultSettingsDraft.routing,
        onProvidersChange: (next: ModelProvidersSettings) => {
          providers = next;
          render();
        },
    ...
    const input = document.querySelector<HTMLInputElement>(
      'input[placeholder="http://provider.local:9810"]',
    )!;
    await act(async () => {
    ...
    expect(document.querySelector(".provider-test-result")?.classList.contains("success")).toBe(
      false,
    );
  ...
  test("returns to idle when the address changes outside the input handler", async () => {
    ...
    expect(
      container.querySelector<HTMLButtonElement>(".provider-card-footer button")?.disabled,
    ).toBe(false);
    expect(container.querySelector(".provider-test-result")?.classList.contains("success")).toBe(
      false,
    );
