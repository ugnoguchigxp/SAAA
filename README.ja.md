...
- 既定の会話経路を使う場合は、プライベートネットワークから接続できるローカル LLM サーバーと `LARM_API_TOKEN`
...
既定のローカル LLM 経路を使う場合は、同じシェルでトークンを環境変数に設定してから起動します。
...
アプリが開いたら、Settings → Model Providers でローカル LLM サーバーのホスト名またはプライベート IP を確認してください。入力するのはホストだけです。接続先の詳細とモデル名はサーバーから取得するため、設定画面には保存しません。

## モデル接続を設定する

### ローカル LLM サーバー

既定の会話経路です。SAAA は設定したホストの接続 API を通じて、会話に使うローカルモデルへの接続を取得します。応答で得た OpenAI 互換の接続先、モデル名、短時間だけ有効な認証情報はメモリ上で使い、各ターンの終了時に接続を解放します。これらの値は SQLite に保存しません。
