...
export type AudioUploadPurpose = "voice-enrollment" | "conversation-asr";
...
export function encodePcm16(samples: Float32Array): Uint8Array {
  if (samples.length > MAX_AUDIO_SAMPLES)
    throw new Error("Recorded audio exceeds the two minute limit.");
  if (samples.some((sample) => !Number.isFinite(sample)))
    throw new Error("Recorded audio contains invalid samples.");
...
export async function stageAudioUpload(
