...
export function findPrimaryRoute(documents: SettingsDocument[]): string {
  ...
  if (
    routing &&
    typeof routing.valueJson.conversationRespond === "object" &&
    routing.valueJson.conversationRespond !== null
  ) {
...
export function updateConversationTimestamp(
  ...
  return {
    ...snapshot,
    conversations: [
      { ...conversation, title: conversation.title ?? title.slice(0, 60), updatedAt: "pending" },
      ...snapshot.conversations.filter((item) => item.id !== conversationId),
    ],
  };
}

export function resolveModelProviderStatus(
  snapshot: AppSnapshot,
): EffectiveRouteSnapshot & { ready: boolean } {
  return {
