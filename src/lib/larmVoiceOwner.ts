...
export class LarmVoiceOwners {
  ...
  constructor(private command: Command, private id: () => string) {}
  current(): LarmVoiceOwner | null { return this.owner; }
  own(conversationId: string): LarmVoiceOwner {
    ...
    const work = async () => {
      ...
      const early = stop().then(() => null, (error: unknown) => error);
      if (starting) {
    ...
    current.ending = work().catch((error) => { current.ending = undefined; throw error; });
  ...
  async prepare(conversationId: string): Promise<LarmVoiceOwner | null> {
    ...
    if (!current.ready) {
      ...
      current.ready = this.command("begin_larm_voice_session", { ownerId: current.id, conversationId })
        .then(() => { if (current.phase === "starting") current.phase = "ready"; });
    }
    try { await current.ready; }
    catch (error) { await this.end(current); throw error; }
