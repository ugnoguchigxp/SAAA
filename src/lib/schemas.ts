...
export const inputActivitySignalSchema = z
  .object({
...
export const calibrationParametersSchema = z
  ...
  .strict()
  .refine((value) => value.lowConfidenceMax < value.classificationMinConfidence, {
    message: "Low-confidence maximum must be lower than classification confidence",
  })
  .refine((value) => value.inputActiveMaxMs < value.inputRecentMaxMs, {
    message: "Input active boundary must be lower than input recent boundary",
  });

export const codexAgentSettingsSchema = z
  .object({
    agentName: z
      .string()
      .trim()
    ...
    conversationRespond: z
      .object({
        ...
        timeoutMs: z
          .number()
          .int()
          .min(MIN_CONVERSATION_TIMEOUT_MS)
          .max(MAX_CONVERSATION_TIMEOUT_MS),
      })
      .strict()
      .superRefine((route, context) => {
        ...
        if (route.source === "harness" && route.primaryProviderId !== null)
          context.addIssue({
            code: "custom",
            message: "Harness source must not reference an individual provider",
            path: ["primaryProviderId"],
          });
      }),
    voiceTranscribe: z
      ...
      .strict()
      .superRefine((route, context) => {
        if (route.source === "provider" && !route.providerId)
          context.addIssue({
            code: "custom",
            message: "Individual ASR source requires a provider",
            path: ["providerId"],
          });
        if (route.source === "harness" && route.providerId !== null)
          context.addIssue({
            code: "custom",
            message: "Harness source must not reference an individual provider",
            path: ["providerId"],
    ...
    voiceSpeak: z
      ...
      .superRefine((route, context) => {
        if (route.source === "provider" && !route.providerId)
          context.addIssue({
            code: "custom",
            message: "Individual TTS source requires a provider",
            path: ["providerId"],
          });
        if (route.source === "harness" && route.providerId !== null)
          context.addIssue({
...
export const voiceSettingsSchema = z
  .object({
    ...
    allowedLanguages: z
      .array(z.enum(ASR_LANGUAGE_CODES))
      .min(1)
      .max(ASR_LANGUAGE_CODES.length)
      .refine(
...
export const situationSettingsSchema = z
  .object({
    enabled: z.boolean(),
...
export function validateSettingsDocuments(documents: unknown[]): void {
  ...
  const enabled = new Map(
    providers.filter((provider) => provider.enabled).map((provider) => [provider.id, provider]),
  );
  ...
  if (
    ...
    routing.conversationRespond.timeoutMs > LEGACY_DYNAMIC_LAN_MAX_REQUEST_TIMEOUT_MS
  ) {
    throw new Error(
  ...
  const asrProvider = routing.voiceTranscribe.providerId
    ...
    : undefined;
  if (routing.voiceTranscribe.source === "provider" && asrProvider?.kind !== "cloud-asr")
    throw new Error("The selected provider does not support ASR");
  const ttsProvider = routing.voiceSpeak.providerId
    ? enabled.get(routing.voiceSpeak.providerId)
    : undefined;
  if (routing.voiceSpeak.source === "provider" && !ttsProvider)
    throw new Error("The selected TTS provider must be enabled");
  if (ttsProvider && !["cloud-tts", "system-tts"].includes(ttsProvider.kind))
    throw new Error("The selected provider does not support TTS");
}
