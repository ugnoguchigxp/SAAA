...
export function SituationReview() {
  ...
  const [reasonCode, setReasonCode] =
    useState<(typeof decisionReasons)[number]>("insufficient-evidence");
  ...
  async function decide(id: string, decision: "accept" | "reject" | "rollback") {
    if (
      workingId ||
      !window.confirm(
        t("review.confirmDecision", {
          decision: t(`review.${decision}`, { defaultValue: decision }),
        }),
      )
    )
      return;
  ...
  return (
    <div className="situation-content">
      ...
      <section className="situation-card">
        ...
        <p>
          {t("review.activeRule")} <strong>{snapshot.activeProfile.ruleVersion}</strong>
        </p>
        <div className="settings-summary-grid">
          <div>
            <span>{t("review.qualitySamples")}</span>
            <strong>{snapshot.quality.sampleCount}</strong>
          </div>
          <div>
            <span>{t("review.pendingFeedback")}</span>
            <strong>{snapshot.feedbackQueue.length}</strong>
          </div>
          <div>
            <span>{t("review.flappingRate")}</span>
            <strong>
              {formatRate(snapshot.quality.flappingRate, t("review.insufficientData"))}
            </strong>
          </div>
          <div>
            <span>{t("review.staleSignalRate")}</span>
            <strong>{formatRate(snapshot.quality.staleRate, t("review.insufficientData"))}</strong>
          </div>
        </div>
        {snapshot.quality.sampleCount < 20 && (
          <p className="settings-help">{t("review.insufficientRates")}</p>
        )}
      </section>
      ...
      <section className="situation-card">
        ...
        <div className="settings-form-grid">
          <Parameter
            label={t("review.classificationConfidence")}
            value={parameters.classificationMinConfidence}
            min={50}
            max={95}
            onChange={(value) =>
              setParameters((current) => ({ ...current, classificationMinConfidence: value }))
            }
          />
          <Parameter
            label={t("review.lowConfidenceMaximum")}
            value={parameters.lowConfidenceMax}
            min={0}
            max={60}
            onChange={(value) =>
              setParameters((current) => ({ ...current, lowConfidenceMax: value }))
            }
          />
          <Parameter
            label={t("review.enterSamples")}
            value={parameters.enterSampleCount}
            min={1}
            max={10}
            onChange={(value) =>
              setParameters((current) => ({ ...current, enterSampleCount: value }))
            }
          />
          <Parameter
            label={t("review.exitSamples")}
            value={parameters.exitSampleCount}
            min={1}
            max={20}
            onChange={(value) =>
              setParameters((current) => ({ ...current, exitSampleCount: value }))
            }
          />
          <Parameter
            label={t("review.cooldown")}
            value={parameters.cooldownMs}
            min={0}
            max={60_000}
            step={500}
            onChange={(value) => setParameters((current) => ({ ...current, cooldownMs: value }))}
          />
          <Parameter
            label={t("review.inputActiveMax")}
            value={parameters.inputActiveMaxMs}
            min={5_000}
            max={120_000}
            step={5_000}
            onChange={(value) =>
              setParameters((current) => ({ ...current, inputActiveMaxMs: value }))
            }
          />
          <Parameter
            label={t("review.inputRecentMax")}
            value={parameters.inputRecentMaxMs}
            min={60_000}
            max={1_800_000}
            step={30_000}
            onChange={(value) =>
              setParameters((current) => ({ ...current, inputRecentMaxMs: value }))
            }
          />
        </div>
        {!inputBoundariesOrdered && <p className="settings-help">{t("review.inputOrderError")}</p>}
        {!parametersValid && inputBoundariesOrdered && (
          <p className="settings-help">{t("review.rangeError")}</p>
        )}
        <button
          className="save-button situation-toggle"
          disabled={!parametersValid || creating || Boolean(workingId)}
          onClick={() => void createCandidate()}
        >
          {creating ? t("review.creating") : t("review.create")}
        </button>
      </section>
      ...
      <section className="situation-card">
        <div className="section-heading">
          ...
          <label className="settings-field">
            {t("review.decisionReason")}
            <select
              value={reasonCode}
              onChange={(event) => setReasonCode(event.currentTarget.value as typeof reasonCode)}
            >
        ...
        <div className="situation-profile-list">
          {snapshot.candidates.map((profile) => (
            <article key={profile.id}>
              <div>
                <strong>{profile.ruleVersion}</strong>
                <span>
                  {t(`review.status.${profile.status}`, { defaultValue: profile.status })}
                </span>
              </div>
              <div className="profile-actions">
                {profile.status === "candidate" && (
                  <>
                    <button
                      ...
                      onClick={() => void replay(profile.id)}
                    >
                      {workingId === profile.id ? t("review.running") : t("review.replay")}
                    </button>
                    <button
                      className="send-button"
                      disabled={Boolean(workingId)}
                      onClick={() => void decide(profile.id, "accept")}
                    >
                      {t("review.accept")}
                    </button>
                    <button
                      ...
                      disabled={Boolean(workingId)}
                      onClick={() => void decide(profile.id, "reject")}
                    >
                      {t("review.reject")}
                    </button>
                  </>
                ...
                {profile.status === "superseded" && (
                  <button
                    className="secondary-button"
                    disabled={Boolean(workingId)}
                    onClick={() => void decide(profile.id, "rollback")}
                  >
                    {t("review.rollback")}
                  </button>
                )}
      ...
      {snapshot.latestRun && (
        <section className="situation-card">
          ...
          {replayMetrics ? (
            <div className="settings-summary-grid">
              <div>
                <span>{t("review.fixture")}</span>
                <strong>{replayMetrics.fixtureSetVersion}</strong>
              </div>
              <div>
                <span>{t("review.samples")}</span>
                <strong>{replayMetrics.sampleCount}</strong>
              </div>
              <div>
                <span>{t("review.candidateMatches")}</span>
                <strong>{replayMetrics.expectedSceneMatches}</strong>
              </div>
              <div>
                <span>{t("review.baselineMatches")}</span>
                <strong>{replayMetrics.baselineExpectedSceneMatches}</strong>
              </div>
              <div>
                <span>{t("review.attentionMatches")}</span>
                <strong>
                  {replayMetrics.expectedAttentionMatches}/{replayMetrics.expectedAttentionSamples}
                </strong>
              </div>
              <div>
                <span>{t("review.baselineAttention")}</span>
                <strong>
                  {replayMetrics.baselineExpectedAttentionMatches}/
                  {replayMetrics.expectedAttentionSamples}
                </strong>
              </div>
            </div>
          ) : (
            <p className="settings-help">{t("review.unreadableMetrics")}</p>
          )}
        </section>
      )}
    </div>
