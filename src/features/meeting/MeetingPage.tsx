...
export function MeetingPage({ voiceSettings, chatVoiceBusy, onStateChanged }: { voiceSettings: VoiceSettings | null; chatVoiceBusy: boolean; onStateChanged: (state: MeetingState) => void }) {
  ...
  return <section className="meeting-page">
    ...
    <div className="route-banner"><strong>Microphone · {meeting.health}</strong><span>System audio: Unavailable in this build</span><span>STT: {voiceSettings?.sttProviderId || "unavailable"} / {voiceSettings?.sttModel || "no model"}</span></div>
