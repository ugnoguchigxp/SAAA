...
export function MeetingPage({ voiceSettings, conversationBusy, onBeforeCapture, onStateChanged }: { voiceSettings: VoiceSettings | null; conversationBusy: boolean; onBeforeCapture: () => Promise<void>; onStateChanged: (state: MeetingState) => void }) {
  ...
  return <section className="meeting-page">
    ...
    <div className="route-banner"><strong>{t("meeting.microphone")} · {localizeStatus(t, meeting.health)}</strong><span>{t("meeting.systemAudioUnavailable")}</span><span>{t("meeting.sttRoute")}</span></div>
    ...
    {meeting.snapshot.state === "completed" && decision === null && <div className="meeting-actions"><button className="send-button" disabled={meeting.working || finalEntries.length === 0} onClick={() => setDecision("save")}>{t("meeting.reviewSave")}</button><button className="stop-button" disabled={meeting.working} onClick={() => setDecision("discard")}>{t("meeting.reviewDiscard")}</button></div>}
    {meeting.snapshot.state === "completed" && decision === "save" && <section className="meeting-decision" aria-label={t("meeting.saveAria")}><h2>{t("meeting.saveTitle")}</h2><dl><div><dt>{t("meeting.target")}</dt><dd>{t("meeting.targetValue")}</dd></div><div><dt>{t("meeting.entries")}</dt><dd>{t("meeting.entriesValue", { count: finalEntries.length })}</dd></div><div><dt>{t("meeting.languages")}</dt><dd>{languages.length ? languages.map((language) => t(`asrLanguages.${language}`, { defaultValue: t("meeting.notDetected") })).join(", ") : t("meeting.notDetected")}</dd></div><div><dt>{t("meeting.audio")}</dt><dd>{t("meeting.audioValue")}</dd></div></dl><div className="meeting-actions"><button className="send-button" disabled={meeting.working || finalEntries.length === 0} onClick={() => void meeting.save()}>{t("meeting.confirmSave")}</button><button className="secondary-button" disabled={meeting.working} onClick={() => setDecision(null)}>{t("common.back")}</button></div></section>}
    {meeting.snapshot.state === "completed" && decision === "discard" && <section className="meeting-decision danger" aria-label={t("meeting.discardAria")}><h2>{t("meeting.discardTitle")}</h2><p>{t("meeting.discardDescription", { count: finalEntries.length })}</p><div className="meeting-actions"><button className="stop-button" disabled={meeting.working} onClick={() => void meeting.discard()}>{t("meeting.confirmDiscard")}</button><button className="secondary-button" disabled={meeting.working} onClick={() => setDecision(null)}>{t("common.back")}</button></div></section>}
