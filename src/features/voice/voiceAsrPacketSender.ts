...
type Operation =
  ...
  | ({ type: "commit"; reason: CommitReason } & Control)
  | ({ type: "stop"; finalizeCurrent: boolean } & Control);
type SenderApi = {
  append: (sequence: number, bytes: Uint8Array) => Promise<void>;
  commit: (reason: CommitReason) => Promise<void>;
  stop: (finalizeCurrent: boolean) => Promise<void>;
};
...
export class VoiceAsrPacketSender {
  ...
  private enqueueControl(type: "commit" | "stop", value: CommitReason | boolean): Promise<void> {
    ...
    if (type === "commit") {
      ...
      this.operations.push({
        type,
        finalizeCurrent: value as boolean,
        resolve: resolveControl,
        reject: rejectControl,
      });
    }
    ...
    try {
      while (this.operations.length) {
        ...
        if (operation.type === "audio") {
          ...
          await this.api.stop(operation.finalizeCurrent);
          operation.resolve();
