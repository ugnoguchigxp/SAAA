...
export function auditCaptureCancelled(sessionId: string, conversationId: string | null) {
  recordAuditEvent({
    component: "microphone",
    eventName: "capture-start-cancelled",
    phase: "terminal",
    outcome: "cancelled",
    correlationId: sessionId,
    conversationId,
    sessionId,
    failureCode: "stale-session",
  });
}

export function auditCaptureFailed(
  sessionId: string,
  conversationId: string | null,
  cause: unknown,
) {
  recordAuditEvent({
    component: cause instanceof MicrophoneCaptureError ? "microphone" : "voice-asr",
    eventName: "capture-start-failed",
    phase: "error",
    outcome: "failure",
    correlationId: sessionId,
    conversationId,
...
export function auditCaptureSuspended(
  ...
  recordAuditEvent({
    component: "microphone",
    eventName: "capture-suspended",
    phase: "state",
    outcome: "success",
    correlationId: sessionId,
    conversationId,
    sessionId,
    attributes: { reasonCode: reason },
  });
}

export function auditVoiceDeliveryBlocked(
  sessionId: string,
  utteranceId: string,
  ...
  recordAuditEvent({
    ...
    eventName: "voice-delivery-blocked",
    phase: "decision",
    outcome: "blocked",
    correlationId: sessionId,
    causationId: utteranceId,
    conversationId,
    sessionId,
    subjectId: utteranceId,
    failureCode: "pending-limit",
    attributes: queueDepth === undefined ? {} : { queueDepth },
  });
}

export function auditVoiceDeliveryDecision(
  ...
  recordAuditEvent({
    component: "conversation",
    eventName: deliveryMode === "queued" ? "voice-utterance-queued" : "voice-utterance-submitted",
    phase: "decision",
    outcome: "success",
    correlationId: utterance.sessionId,
    causationId: utterance.utteranceId,
    conversationId: utterance.conversationId,
    sessionId: utterance.sessionId,
    subjectId: utterance.utteranceId,
    attributes: { deliveryMode, ...(queueDepth === undefined ? {} : { queueDepth }) },
  });
}
...
export function auditVoiceDeliverySettlement(
  ...
  return (delivered) => {
    recordAuditEvent({
      component: "conversation",
      eventName: "voice-delivery-finished",
      phase: "terminal",
      ...
      sessionId: utterance.sessionId,
      subjectId: utterance.utteranceId,
      failureCode: delivered ? null : "turn-not-accepted",
    });
    settle(delivered);
  };
}
