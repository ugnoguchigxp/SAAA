...
export class InstanceStateStore {
  ...
  constructor(
    private write = uiApi.state,
    private read = uiApi.load,
  ) {}
  initialize(instance: UiInstance) {
    ...
    if (!entry.dirty && !entry.pending && instance.stateVersion > entry.version) {
      entry.value = instance.state;
      entry.base = instance.state;
      entry.version = instance.stateVersion;
      entry.error = false;
  ...
  update(id: string, key: string, value: State[string]) {
    const entry = this.get(id);
    entry.value = { ...entry.value, [key]: value };
    entry.dirty = true;
    entry.listeners.forEach((fn) => fn());
    void this.flush(id);
  }
  async flush(id: string): Promise<void> {
    ...
    entry.pending = (async () => {
      ...
      while (entry.dirty) {
        ...
        entry.dirty = false;
        try {
          entry.version = await this.write(id, entry.version, value);
          entry.base = value;
          entry.error = false;
        } catch (cause) {
          entry.dirty = true;
          if (!rebased && String(cause).includes("UI state conflict")) {
            ...
            try {
              ...
              const changes = Object.fromEntries(
                Object.entries(entry.value).filter(([key, value]) => value !== entry.base[key]),
              );
              entry.base = remote.state;
              entry.version = remote.stateVersion;
              entry.value = { ...remote.state, ...changes };
              continue;
            } catch {
              /* Preserve local edits if reloading also fails. */
            }
          }
          entry.error = true;
          break;
