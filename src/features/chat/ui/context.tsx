...
export function useUiContext() {
  ...
  if (!value) throw new Error("Missing UI host");
  return value;
}
export function useUiData(source: string) {
  ...
  return instance.mode === "snapshot"
    ? { data: instance.snapshots[source], loading: false }
    : { ...snapshot, data: snapshot.data ?? previous.current };
}
export function useUiField(key: string, fallback: string | number | boolean) {
  ...
  return [
    state[key] ?? fallback,
    (value: string | number | boolean) => uiStates.update(instance.id, key, value),
  ] as const;
}
