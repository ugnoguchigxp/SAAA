...
function LoadedUi({
  ...
  const stateError = useSyncExternalStore(
    useCallback((listener) => uiStates.subscribe(instance.id, listener), [instance.id]),
    () => uiStates.get(instance.id).error,
  );
  async function save() {
    setSaving(true);
    try {
      await uiApi.save(instance.id, name, instance.summary);
      setOutcome("genui.savedDone");
    } catch {
  ...
  async function snapshot() {
    setSaving(true);
    try {
      await uiApi.snapshot(instance.id);
  ...
  return (
    <UiContext.Provider value={{ instance, conversationId, active, enabled }}>
      <header className="ui-header">
        <strong>{instance.name ?? instance.summary}</strong>
        <span>
      ...
      {stateError && (
        <p role="alert">
          {t("genui.stateFailed")}{" "}
          <button onClick={() => void uiStates.flush(instance.id)}>{t("genui.retry")}</button>
        </p>
      )}
      <footer className="ui-footer">
        ...
        >
          ...
          <button disabled={!enabled || saving || !name.trim()}>{t("genui.save")}</button>
        </form>
        {instance.mode === "live" && (
          <button disabled={!enabled || saving || !active} onClick={() => void snapshot()}>
            {t("genui.snapshotAction")}
          </button>
        )}
        {outcome && <p role="status">{t(outcome)}</p>}
        <small>{t("genui.editHint")}</small>
      </footer>
    </UiContext.Provider>
  );
}
export default function InlineUi({
  ...
  const { t } = useTranslation();
  const [instance, setInstance] = useState<UiInstance>();
  const [failed, setFailed] = useState(false);
  const [retry, setRetry] = useState(0);
  const { ref, active } = useUiVisibility();
  ...
  const fallback = (
    <div>
      <p>{summary}</p>
      <p role="status">{t("genui.unavailable")}</p>
      <button onClick={() => setRetry((value) => value + 1)}>{t("genui.retry")}</button>
    </div>
  );
  return (
    <div className="inline-ui" ref={ref} aria-label={summary}>
      <UiBoundary key={`${instanceId}:${retry}`} fallback={fallback}>
        {failed ? (
          fallback
        ) : instance ? (
          <LoadedUi
