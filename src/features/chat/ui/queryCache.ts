...
type Entry = { value: QuerySnapshot; subscribers: Map<symbol, () => Promise<UiData>>; listeners: Set<() => void>; timer?: ReturnType<typeof setTimeout>; pending?: Promise<void>; generation: number };
...
export class UiQueryCache {
  ...
  snapshot(key: string): QuerySnapshot { return this.entries.get(key)?.value ?? EMPTY; }
  subscribe(key: string, fetch: () => Promise<UiData>, listener: () => void): () => void {
    let entry = this.entries.get(key);
    if (!entry) { entry = { value: EMPTY, subscribers: new Map(), listeners: new Set(), generation: 0 }; this.entries.set(key, entry); }
    const token = Symbol(); entry.subscribers.set(token, fetch); entry.listeners.add(listener);
    if (entry.subscribers.size === 1) void this.refresh(key);
    return () => {
      entry.subscribers.delete(token); entry.listeners.delete(listener);
      if (!entry.subscribers.size) {
        clearTimeout(entry.timer); entry.generation++;
        this.entries.delete(key); // a late result cannot restore a disposed entry
      }
    };
  }
  async refresh(key: string): Promise<void> {
    const entry = this.entries.get(key); if (!entry || !entry.subscribers.size) return;
    ...
    entry.pending = Promise.resolve().then(async () => {
      ...
      try {
        ...
        if (generation === entry.generation) entry.value = { data: entry.value.data, error: 'unavailable', loading: false };
      } finally {
