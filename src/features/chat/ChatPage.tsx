...
export function ChatPage({
  ...
  onLoadOlderMessages,
  hasNewerMessages = false, loadingNewerMessages = false, onLoadNewerMessages,
  streamingText,
  interimTranscript,
  voiceState,
  listeningEnabled,
  ...
  }, [messages, streamingText, interimTranscript, asrProjection, runtimeActivity, hasNewerMessages]);
  async function handleMessageAreaScroll() {
    ...
    if (messageArea.scrollHeight - messageArea.scrollTop - messageArea.clientHeight < 100 && hasNewerMessages && !loadingNewerMessages) {
      await onLoadNewerMessages?.(); return;
    }
    if (messageArea.scrollTop < 100 && hasMoreMessages && !loadingOlderMessages) await onLoadOlderMessages();
  }
  return <section className="chat-panel">
    <header className="topbar"><div><p className="eyebrow">{t("chat.eyebrow")}</p><h1>{t("chat.title")}</h1></div><div className="topbar-status"><UiControls conversationId={selectedConversation?.id} /><span className="status-pill local-status"><span className="status-dot" />{modelProviderStatus.location === "cloud" ? t("chat.cloudProcessing") : modelProviderStatus.location === "local" ? t("chat.localProcessing") : t("chat.processingNotSelected")}</span><button className={!modelProviderStatus.ready ? "status-pill provider-status warning" : "status-pill provider-status"} onClick={onOpenSettings}><AppIcon name="model" />{localizeProviderLabel(t, modelProviderStatus.label)}{modelProviderStatus.state === "failed" ? t("chat.failedSuffix") : modelProviderStatus.state === "unchecked" ? t("chat.uncheckedSuffix") : modelProviderStatus.fallbackUsed ? t("chat.fallbackSuffix") : ""}</button></div></header>
