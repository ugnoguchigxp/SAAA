...
import type { VoiceCaptureState } from "../voice/usePushToTalk";
...
export function ChatPage({
  ...
  modelProviderStatus: { ready: boolean; label: string };
  onOpenSettings: () => void;
  onOpenMeeting: () => void;
  ...
  return <section className="chat-panel">
    ...
    <form className="composer" onSubmit={onSubmit}><div className="composer-row"><button className={voiceState === "recording" ? "voice-button recording" : "voice-button"} type="button" aria-label={voiceStarting ? "マイクを準備中" : voiceState === "recording" ? "録音を停止" : voiceState === "transcribing" ? "文字起こしを停止" : "音声入力"} title={voiceStarting ? "マイクを準備中" : voiceState === "recording" ? "録音を停止（無音で自動送信）" : voiceState === "transcribing" ? "文字起こしを停止" : "音声入力"} onClick={onToggleVoice} disabled={voiceStarting || meetingActive || (Boolean(activeRunId) && !filterEnabled)}><AppIcon name={voiceState === "recording" || voiceState === "transcribing" ? "stop" : "mic"} /></button><textarea rows={1} aria-label="Message" onChange={(event) => onComposerChange(event.currentTarget.value)} onKeyDown={(event) => { if ((event.metaKey || event.ctrlKey) && event.key === "Enter") event.currentTarget.form?.requestSubmit(); }} placeholder="SAAAにメッセージ" value={composer} disabled={Boolean(activeRunId)} /><div className="composer-end">{activeRunId ? <button className="stop-button composer-stop" type="button" onClick={onStopRun}><AppIcon name="stop" /><span>停止</span></button> : <button className="send-button" type="submit" aria-label="送信" disabled={!composer.trim() || !selectedConversation || voiceBusy}><AppIcon name="send" /></button>}</div></div><div className="composer-meta">{voiceState === "recording" && <span className="composer-hint">話し終えて{DEFAULT_VOICE_SILENCE_TIMEOUT_MS / 1_000}秒待つと自動送信します。{filterEnabled ? " 本人フィルター中は応答・読み上げ中も待ち受けます。" : ""}</span>}{activeTtsRunId && <button className="text-button" type="button" onClick={onStopSpeech}>読み上げを停止</button>}{error && lastPrompt && !activeRunId && <button className="text-button" type="button" onClick={onRetry}>再試行</button>}</div></form>
