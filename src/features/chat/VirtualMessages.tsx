...
export function VirtualMessages({
  ...
  const virtual = useVirtualizer({
    ...
    estimateSize: (index) =>
      heights.current.get(messages[index].id) ??
      (messages[index].parts?.some((part) => part.type === "ui") ? 540 : 140),
    overscan: 3,
    useFlushSync: false,
    anchorTo: "end",
    ...
    rangeExtractor: (range) => {
      const items = defaultRangeExtractor(range);
      const index = messages.findIndex((message) => message.id === focused);
    ...
    measureElement: (element) => {
      ...
      if (id) {
        ...
        if (heights.current.size > 300)
          heights.current.delete(heights.current.keys().next().value!);
      }
      return height;
    },
  });
  ...
  useEffect(() => {
    ...
    for (const key of virtual.itemSizeCache.keys())
      if (!retained.has(String(key))) virtual.itemSizeCache.delete(key);
  }, [messages, virtual]);
    ...
    >
      {virtual.getVirtualItems().map((item) => (
        <div
          ...
          onFocusCapture={() => setFocused(messages[item.index].id)}
          onBlurCapture={(event) => {
