...
const persisted = (message: ConversationMessage) =>
  !message.id.startsWith("pending_") && !message.id.startsWith("streaming_");
...
export class MessageHistoryStore {
  ...
  setMessages = (
    update: ConversationMessage[] | ((current: ConversationMessage[]) => ConversationMessage[]),
  ) => {
    const messages = typeof update === "function" ? update(this.value.messages) : update;
    this.update({
      messages: messages.slice(-HISTORY_LIMIT),
      hasMoreMessages: this.value.hasMoreMessages || messages.length > HISTORY_LIMIT,
    });
  };
  reset = (conversation: string | null) => {
  ...
  private applyLatest(page: MessagePage) {
    ...
    const overlap =
      page.messages.length > 0 && current.some((old) => old.id === page.messages[0].id);
    const messages = overlap ? mergeMessageWindow(current, page.messages, "after") : page.messages;
    const hasMoreMessages =
      messages[0]?.id === page.messages[0]?.id
        ? page.hasMore
        : this.value.hasMoreMessages || messages[0]?.id !== current[0]?.id;
  ...
  load = async (direction: Direction) => {
    ...
    const generation = this.generation;
    this.update(
      direction === "before" ? { loadingOlderMessages: true } : { loadingNewerMessages: true },
    );
    try {
      ...
      this.update(
        direction === "before"
          ? {
              ...
              hasMoreMessages: page.hasMore,
              hasNewerMessages:
                this.value.hasNewerMessages ||
                messages[messages.length - 1]?.id !== previous[previous.length - 1]?.id,
            }
          : {
              messages,
